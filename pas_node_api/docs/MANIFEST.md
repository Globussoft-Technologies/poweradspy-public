...
| **nasClient** | `storeInNas(type, filePath, id, network, keyBaseName)` | type → subfolder; network → NAS folder prefix (fb/insta/pint/gt/yt/…); filename = `keyBaseName` (the id). Bearer token, bucket pas-dev/pas-prod. Returns the stored `path` or `/DefaultImage.jpg`. |
| **mediaUpload** | `uploadImage/Thumbnail/Video/PostOwner(url,id,network)`, `uploadMultimedia`, `mediaIssueWarning(paths,type)` | downloads to temp → storeInNas → cleanup. NAS-only (PHP S3 was commented out — no AWS SDK needed). |
...
7. **NAS (media.globussoft.com)** — per the Media Upload API doc: key = `<networkFolder>/<typeSubfolder><YYYYMM>/<id>.jpg`; Bearer token; bucket pas-dev/pas-prod; response `path` (e.g. `/pas-dev/stream/fb/adImage/202605/130721.jpg`) is what we store. **Filename = the entity id** (ad id for ad media, post_owner_id for post-owner image) — NOT random. Network→folder map lives in `nasClient.NAS_KEY_PREFIX`. **Endpoints + paths are config-driven** (`nas.mediaUrl`+`mediaUploadPath`, `nas.videoUrl`+`videoUploadPath`) — never hardcode routes.
   - **Video files** use the SEPARATE video endpoint (`videoUrl`+`videoUploadPath`, multipart network/file/adid); the path is stored ONLY in ES `nas_video_url` (PHP never stored it in SQL). For a VIDEO ad, `facebook_ad_variants.image_url` holds the THUMBNAIL, `image_url_original` the original video URL.
   - **Carousel / other_multimedia** → uploaded in parallel, stored in `facebook_ad_image_video.ad_image_video` (JSON array) AND ES `othermedia`.
...
10. **Per-network column quirks (Instagram, vs Facebook)** — confirm each network's real schema:
    ...
    - ES index `instagram_search_mix`, field keys prefixed `instagram_*`, date fields verified (`instagram_ad.post_date`/`last_seen`/`firstSeenOn*` = `yyyy-MM-dd HH:mm:ss`, `instagram_ad_domain.domain_registered_date` = `yyyy-MM-dd`). **`instagram_ad.created_date` has NO explicit format → ES default (strict_date_optional_time) → must be sent as ISO `yyyy-MM-ddTHH:mm:ss` (esDocBuilder `'iso'` kind), NOT the space form.**
