...
`native`, and `pinterest`. It is read-only and does not manage DS workers.
...
`google_text_ad`, `native_ad`, and `pinterest_ad`. SQL is necessary because not
every search index carries `created_date`. Elasticsearch then applies the shared
`getDisplayableMediaFilter` dashboard rules and the same response-normalization
and SQL-fallback helpers as `getDescriptionDetails`.
...
Production `EXPLAIN FORMAT=JSON` checks confirmed that every platform already has
an effective covering keyset plan: `range` access through its `created_date`
secondary index, primary-key `id` ordering supplied by InnoDB, and no filesort.
No new SQL index is required for any of the six platforms.
