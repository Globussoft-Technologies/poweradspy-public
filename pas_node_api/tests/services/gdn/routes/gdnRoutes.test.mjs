...
function mkCtrlMock(filename, exportName) {
  const p = require.resolve(`../../../../src/services/gdn/controllers/${filename}`);
...
const insightsExports = mkCtrlMock("adInsightsController", [
  "getGdnAdCountry", "getGdnOutgoings", "getAdvertiserCountryData", "getAdvertiserInsightsByDateRange",
]);
...
const { createGdnRoutes } = require(
  "../../../../src/services/gdn/routes/gdnRoutes"
);
...
beforeEach(() => {
  ...
  for (const fn of [searchAds, getAdDetails, getAdsCount, ...Object.values(hideExports), ...Object.values(insightsExports)]) fn.mockReset();
});
...
describe("gdnRoutes > registration", () => {
  it("registers expected routes", () => {
    const router = createGdnRoutes(svc);
    for (const p of [
      "/ads/search", "/ads/detail", "/ads/hide_ads",
      "/ads/getHiddenPostOwners", "/ads/un-hide", "/ads/getGdnAdCountry",
      "/ads/getGdnOutgoings", "/ads/getAdvertiserCountryData", "/ads/getAdvertiserInsightsByDateRange",
    ]) expect(router.routes.post[p]).toBeDefined();
    expect(router.routes.get["/ads/count"]).toBeDefined();
  });
});

describe("gdnRoutes > /ads/search", () => {
  it("200 → success", async () => {
    searchAds.mockResolvedValue({ code: 200, data: [], total: 0 });
    await lastHandler(createGdnRoutes(svc), "post", "/ads/search")({ body: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("non-200 passes through", async () => {
    searchAds.mockResolvedValue({ code: 503 });
    const res = mkRes();
    await lastHandler(createGdnRoutes(svc), "post", "/ads/search")({ body: {} }, res);
    expect(res.statusCode).toBe(503);
  });
});

describe("gdnRoutes > /ads/detail", () => {
  it("200 → success w/ country meta", async () => {
    getAdDetails.mockResolvedValue({ code: 200, data: [{}], country: ["US"], builtwithStatusCode: 200 });
    await lastHandler(createGdnRoutes(svc), "post", "/ads/detail")({ body: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("non-200 passes through", async () => {
    getAdDetails.mockResolvedValue({ code: 404 });
    const res = mkRes();
    await lastHandler(createGdnRoutes(svc), "post", "/ads/detail")({ body: {} }, res);
    expect(res.statusCode).toBe(404);
  });
});

describe("gdnRoutes > /ads/count (GET)", () => {
  it("200 → success", async () => {
    getAdsCount.mockResolvedValue({ code: 200, data: { count: 5 } });
    await lastHandler(createGdnRoutes(svc), "get", "/ads/count")({ query: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("non-200 passes through", async () => {
    getAdsCount.mockResolvedValue({ code: 500 });
    const res = mkRes();
    await lastHandler(createGdnRoutes(svc), "get", "/ads/count")({ query: {} }, res);
    expect(res.statusCode).toBe(500);
  });
});

describe("gdnRoutes > pass-through delegates", () => {
  const direct = [
    ["/ads/hide_ads", hideExports.hideAds],
    ["/ads/getHiddenPostOwners", hideExports.getHiddenPostOwners],
    ["/ads/un-hide", hideExports.unHide],
    ["/ads/getGdnAdCountry", insightsExports.getGdnAdCountry],
    ["/ads/getGdnOutgoings", insightsExports.getGdnOutgoings],
  ];
  it.each(direct)("%s 200 path", async (path, ctrl) => {
    ctrl.mockResolvedValue({ code: 200, data: [] });
    const res = mkRes();
    await lastHandler(createGdnRoutes(svc), "post", path)({ body: {} }, res);
