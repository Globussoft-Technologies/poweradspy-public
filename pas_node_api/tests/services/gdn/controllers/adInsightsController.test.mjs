...
} = require("../../../../src/services/gdn/controllers/adInsightsController");
...
describe("services/gdn/controllers/adInsightsController > getGdnAdCountry", () => {
  it("401 when gdn_ad_id missing", async () => {
    expect(await getGdnAdCountry({ body: { user_id: "u" }, query: {} }, {}, fakeLogger))
      .toEqual({ code: 401, message: "Missing parameters: gdn_ad_id and user_id are required" });
  });
  it("401 when user_id missing", async () => {
    expect(await getGdnAdCountry({ body: { gdn_ad_id: "1" }, query: {} }, {}, fakeLogger))
      .toEqual({ code: 401, message: "Missing parameters: gdn_ad_id and user_id are required" });
  });
  it("503 when neither SQL nor elastic available", async () => {
    expect(await getGdnAdCountry(
      { body: { gdn_ad_id: "1", user_id: "u" }, query: {} }, { sql: null, elastic: null }, fakeLogger
    )).toEqual({ code: 503, message: "Neither SQL nor Elasticsearch available" });
  });
  it("401 'Something Went Wrong' when no rows", async () => {
    const db = { sql: { query: vi.fn(async () => []) } };
    expect(await getGdnAdCountry(
      { body: { gdn_ad_id: "1", user_id: "u" }, query: {} }, db, fakeLogger
    )).toEqual({ code: 401, message: "Something Went Wrong" });
  });
  it("401 when null rows", async () => {
  ...
  it("SQL throw triggers ES fallback (no elastic → 503)", async () => {
    const db = { sql: { query: vi.fn(async () => { throw new Error("db-down"); }) } };
    const out = await getGdnAdCountry(
      { body: { gdn_ad_id: "1", user_id: "u" }, query: {} }, db, fakeLogger
    );
    expect(out).toEqual({ code: 503, message: "Neither SQL nor Elasticsearch available" });
    expect(fakeLogger.warn).toHaveBeenCalled();
  });

  it("ES fallback: returns countries from ES source", async () => {
    const db = {
      sql: { query: vi.fn(async () => { throw new Error("db-down"); }) },
      elastic: { indexName: "gdn", search: vi.fn(async () => ({ hits: { hits: [{ _source: {
        "gdn_country_only.country": ["germany", "france"],
      }}]}}))},
    };
    const out = await getGdnAdCountry(
      { body: { gdn_ad_id: "1", user_id: "u" }, query: {} }, db, fakeLogger
  ...
  it("ES fallback: single string country normalized to array", async () => {
    const db = {
      sql: null,
      elastic: { search: vi.fn(async () => ({ hits: { hits: [{ _source: {
        "gdn_country_only.country": "japan",
      }}]}}))},
    };
    const out = await getGdnAdCountry(
      { body: { gdn_ad_id: "1", user_id: "u" }, query: {} }, db, fakeLogger
  ...
  it("ES fallback: hit missing country field → 401", async () => {
    const db = {
      sql: null,
      elastic: { search: vi.fn(async () => ({ hits: { hits: [{ _source: {} }] } })) },
    };
  ...
  it("ES fallback: body.hits fallback shape", async () => {
    const db = {
      sql: null,
      elastic: { search: vi.fn(async () => ({ body: { hits: { hits: [{ _source: {
        "gdn_country_only.country": ["italy"]
      }}]}}}))},
    };
    expect((await getGdnAdCountry(
      { body: { gdn_ad_id: "1", user_id: "u" }, query: {} }, db, fakeLogger
  ...
  it("ES fallback: search throws → 500", async () => {
    const db = {
      sql: null,
  ...
  it("ES fallback uses default 'gdn_search_mix' index", async () => {
    const db = {
      sql: null,
      elastic: { search: vi.fn(async () => ({ hits: { hits: [] } })) },
    };
    await getGdnAdCountry(
      { body: { gdn_ad_id: "1", user_id: "u" }, query: {} }, db, fakeLogger
    );
...
describe("services/gdn/controllers/adInsightsController > getGdnOutgoings", () => {
  it("401 when gdn_ad_id missing", async () => {
    expect(await getGdnOutgoings({ body: {}, query: {} }, {}, fakeLogger))
      .toEqual({ code: 401, message: "Missing parameters: gdn_ad_id is required" });
  });
  it("503 when db.sql missing", async () => {
    expect(await getGdnOutgoings(
      { body: { gdn_ad_id: "1" }, query: {} }, { sql: null }, fakeLogger
  ...
  it("200 with URL rows", async () => {
    ...
    const out = await getGdnOutgoings(
...
describe("services/gdn/controllers/adInsightsController > getAdvertiserCountryData", () => {
  ...
  it("401 when gdn_ad_id missing", async () => {
    expect(await getAdvertiserCountryData({ body: {}, query: {} }, {}, fakeLogger))
      .toEqual({ code: 401, message: "Missing gdn_ad_id", data: null });
  });
  ...
  it("explicit p.year overrides ad_last_seen", async () => {
    ...
    const out = await getAdvertiserCountryData(
      { body: { gdn_ad_id: "1", year: 2020 }, query: {} }, db, fakeLogger
    );
  ...
  it("200 with aggregated country data (docvalue_fields shape)", async () => {
    const db = mkDb({
      ...
      esHits: [
        { fields: { "gdn_ad.id": [1], "gdn_country_only.country.keyword": ["germany", "Czechia"] } },
        { fields: { "gdn_ad.id": [2], "gdn_country_only.country.keyword": ["germany"] } },
      ],
      countryRows: [
  ...
  it("supports _source shape (date-range variant)", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [
        { _source: { "gdn_ad.id": 1, "gdn_country_only.country": "italy" } },
      ],
      countryRows: [],
    });
    const out = await getAdvertiserCountryData(
  ...
  it("non-array country normalized to single-element array", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [{ fields: { "gdn_ad.id": [1], "gdn_country_only.country.keyword": "japan" } }],
    });
    const out = await getAdvertiserCountryData(
      { body: { gdn_ad_id: "1" }, query: {} }, db, fakeLogger
    );
  ...
  it("hit missing adId is skipped", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [
        { fields: { "gdn_country_only.country.keyword": ["x"] } }, // no ad id
        { fields: { "gdn_ad.id": [1], "gdn_country_only.country.keyword": ["spain"] } },
      ],
    });
    const out = await getAdvertiserCountryData(
      { body: { gdn_ad_id: "1" }, query: {} }, db, fakeLogger
  ...
  it("ES returns body.hits shape → fallback fires (line 298 right operand)", async () => {
    let sqlCall = 0;
    const db = {
      ...
      elastic: {
        search: vi.fn(async (params) => {
          ...
          return { body: { hits: { hits: [
            { fields: { "gdn_ad.id": [1], "gdn_country_only.country.keyword": ["germany"] } },
          ]}}};
        }),
      },
    };
  ...
  it("batchCountryLookup SQL throw → empty isoMap, country names used as-is", async () => {
    ...
    const db = {
      ...
      elastic: { search: vi.fn(async () => ({ hits: { hits: [
        { fields: { "gdn_ad.id": [1], "gdn_country_only.country.keyword": ["france"] } }
      ]}}))},
    };
    const out = await getAdvertiserCountryData(
      { body: { gdn_ad_id: "1" }, query: {} }, db, fakeLogger
  ...
  it("hit with no countries is skipped", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [
        { fields: { "gdn_ad.id": [1] /* no country */ } },
        { fields: { "gdn_ad.id": [2], "gdn_country_only.country.keyword": ["spain"] } },
      ],
    });
    const out = await getAdvertiserCountryData(
      { body: { gdn_ad_id: "1" }, query: {} }, db, fakeLogger
  ...
  it("countryMap with falsy country entries skipped", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [
        { fields: { "gdn_ad.id": [1], "gdn_country_only.country.keyword": ["", null, "germany"] } },
      ],
    });
    const out = await getAdvertiserCountryData(
      { body: { gdn_ad_id: "1" }, query: {} }, db, fakeLogger
  ...
  it("aggregateCountryData returns null when no country buckets produced", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [{ fields: { "gdn_ad.id": [1] /* no country at all */ } }],
    });
    const out = await getAdvertiserCountryData(
      { body: { gdn_ad_id: "1" }, query: {} }, db, fakeLogger
    );
...
describe("services/gdn/controllers/adInsightsController > getAdvertiserInsightsByDateRange", () => {
  ...
  it("country type: 200 with aggregated data", async () => {
    ...
    const db = {
      ...
      elastic: { search: vi.fn(async () => ({ hits: { hits: [
        { _source: { "gdn_ad.id": 1, "gdn_country_only.country": "germany" } }
      ]}}))},
    };
    const out = await getAdvertiserInsightsByDateRange(
  ...
  it("country type: data null fallback to [] in response", async () => {
    ...
    const db = {
      ...
      elastic: { search: vi.fn(async () => ({ hits: { hits: [
        { _source: { "gdn_ad.id": 1 /* no country */ } }
      ]}}))},
    };
    const out = await getAdvertiserInsightsByDateRange(
      { body: { post_owner_id: 5, from_date: "2024-01-01", to_date: "2024-12-31" }, query: {} }, db, fakeLogger
  ...
  it("body.hits fallback in ES result", async () => {
    ...
    const db = {
      ...
      elastic: { search: vi.fn(async () => ({
        body: { hits: { hits: [{ _source: { "gdn_ad.id": 1, "gdn_country_only.country": "japan" } }] } }
      }))},
    };
    const out = await getAdvertiserInsightsByDateRange(
      { body: { post_owner_id: 5, from_date: "2024-01-01", to_date: "2024-12-31" }, query: {} }, db, fakeLogger
