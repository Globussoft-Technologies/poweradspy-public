...
const { hideAds, getHiddenPostOwners, unHide } = require(
  "../../../../src/services/gdn/controllers/hideAdsController"
);
...
describe("services/gdn/controllers/hideAdsController > hideAds", () => {
...
describe("services/gdn/controllers/hideAdsController > getHiddenPostOwners", () => {
  it("400 when user_id missing", async () => {
...
describe("services/gdn/controllers/hideAdsController > unHide", () => {
  it("400 when user_id missing", async () => {
  ...
  it("500 + logger.error on SQL throw", async () => {
    ...
    expect(out).toEqual({ code: 500, message: "Error in GDN unHide", error: "delete-fail" });
    expect(fakeLogger.error).toHaveBeenCalled();
  });
});
