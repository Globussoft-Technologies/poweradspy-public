...
// ── Mock GoogleSearchQueryBuilder (`new`-invoked, fluent) ──────────────
...
const { searchAds } = require(
  "../../../../src/services/google/controllers/adSearchController"
);
...
describe("services/google/controllers/adSearchController > validation routing", () => {
...
describe("services/google/controllers/adSearchController > searchFavoriteAds", () => {
  ...
  it("with db.elastic: enrichAndFilterRows drops IMAGE without PowerAdspy and keeps the rest", async () => {
    ...
    const db = {
      ...
      elastic: {
        indexName: "google_search_mix",
        search: vi.fn(async () => ({
          hits: { hits: [
            { _source: { id: 1, new_nas_image_url: "https://x/PowerAdspy/img.png" } },
            { _source: { id: 2 } },
            { _source: { id: 3 } },
          ]},
        })),
  ...
  it("enrichAndFilterRows: ES returns body.hits shape (line 77 right operand)", async () => {
    let sqlCall = 0;
    const db = {
      sql: { query: vi.fn(async () => {
        sqlCall++;
...
describe("services/google/controllers/adSearchController > searchHiddenAds", () => {
  it("503 when sql missing", async () => {
...
describe("services/google/controllers/adSearchController > regular searchAds", () => {
  ...
  it("happy path: ES hits → SQL enrich → cleanAdsData", async () => {
    const esHits = [
      { _source: { id: 1, new_nas_image_url: "https://x/nas.png", days_running: 7,
                   country: ["US", "IN"] } },
    ];
    const db = {
      elastic: { indexName: "google_search_mix", search: vi.fn(async () => mkEsHits(esHits)) },
    ...
    expect(out.data[0].country).toBe("US, IN"); // array joined per source line 333
  });
  ...
  it("ES uses body.hits fallback + total as number", async () => {
    const db = {
      elastic: { search: vi.fn(async () => ({ body: { hits: { hits: [{ _source: { "id": 1 } }], total: 1 } } })) },
    };
    const out = await searchAds({ body: { user_id: "u" }, query: {} }, db, fakeLogger);
    expect(out.total).toBe(1);
  });
  ...
  it("SQL fetch failure falls back to ES sources", async () => {
    const esHits = [{ _id: "i-1", _source: { "id": 1, foo: "bar" } }];
    const db = {
  ...
  it("slow request (>1000ms) → logger.warn 'Slow Google search request' (line 357)", async () => {
    // Spy on Date.now to make totalMs > 1000. The SUT calls Date.now() at
    // multiple points (tStart, tEs, after es, tSql, after sql, then totalMs).
    // We let the first few resolve to small deltas (so esElapsed/sqlElapsed
    // stay sane) and the LAST call jump to >1000ms past tStart so the
    // totalMs branch fires.
    let callIdx = 0;
    const real = Date.now;
    const dateSpy = vi.spyOn(Date, "now").mockImplementation(() => {
      callIdx++;
      // 1=tStart=0, 2=tEs=10, 3=tEs-end=20, 4=tSql=30, 5=tSql-end=40,
      // 6+=totalMs-Date.now → 5000 so total_ms=5000 > 1000
      return [0, 10, 20, 30, 40][callIdx - 1] ?? 5000;
    });
    try {
      const esHits = [{ _source: { id: 1 } }];
      const db = {
        elastic: { indexName: "google_search_mix", search: vi.fn(async () => mkEsHits(esHits)) },
        sql: { query: vi.fn(async () => [{ ad_id: 1, id: 1, type: "TEXT" }]) },
      };
      ...
      expect(fakeLogger.warn).toHaveBeenCalledWith(
        "Slow Google search request",
        expect.objectContaining({ total_ms: expect.any(Number) }),
      );
    } finally {
      dateSpy.mockRestore();
    }
  ...
  it("no db.sql → uses ES sources directly", async () => {
    const esHits = [{ _source: { "id": 1, foo: "bar" } }];
  ...
  it("platform 18 returns the compact media contract", async () => {
    const esHits = [{
      _source: {
        ...
        post_date: null,
        first_seen: null,
        last_shown: "2026-07-22T00:00:00Z",
        lang_detect: null,
        title: "",
        text: "",
  ...
  it("invokes builder setters for filter params", async () => {
    ...
    await searchAds({
      body: {
        ...
        type: "IMAGE", call_to_action: "Buy", target_keywords: "kw", tags: "tag1", lang: "en", ad_position: "FEED",
        ad_sub_position: "TOP", gender: "M", lower_age: "18", upper_age: "65",
  ...
  it("ES hit without id falls back to _id key", async () => {
    const esHits = [{ _id: "alt-id-7", _source: { /* no id */ foo: "bar" } }];
    const db = {
  ...
  it("ES src missing new_nas_image_url + days_running (false-side branches)", async () => {
    const esHits = [{ _source: { "id": 1 /* no new_nas_image_url, no days_running */ } }];
    const db = {
      elastic: { search: vi.fn(async () => ({ hits: { hits: esHits, total: { value: 1 } } })) },
      sql: { query: vi.fn(async () => [{ ad_id: 1, type: "TEXT" }]) },
    };
  ...
  it("engagement range filters: setLikes/Comments/Dislikes/Views/AdBudget invoked when arrays present (lines 253-257)", async () => {
    const db = {
      elastic: { indexName: "google_search_mix", search: vi.fn(async () => ({ hits: { hits: [], total: { value: 0 } } })) },
    };
    await searchAds({
      body: {
        user_id: "u",
    ...
    // Did not throw — all 5 range setters were invoked
    expect(db.elastic.search).toHaveBeenCalled();
  });

  it("dedupeRows: duplicate ad_id is dropped (line 60)", async () => {
    // Force the favourite path which calls dedupeRows on SQL results.
    // Returning rows with duplicate ad_id exercises the filter callback's "seen" branch.
    const db = {
      sql: { query: vi.fn(async () => [
        // First call: SELECT favorited ad_ids
        { ad_id: 1 }, { ad_id: 2 },
      ]).mockResolvedValueOnce([{ ad_id: 1 }, { ad_id: 2 }])
        // Second call: getAds — duplicate ad_id triggers dedupe
        .mockResolvedValueOnce([
          { id: 10, ad_id: 1, type: "TEXT" },
          { id: 11, ad_id: 1, type: "TEXT" }, // duplicate → dropped
          { id: 12, ad_id: 2, type: "TEXT" },
        ]),
      },
    };
    const out = await searchAds({ body: { user_id: "u", favorite: "true" }, query: {} }, db, fakeLogger);
    // Either 200 with data or some failure shape; key thing is dedupeRows was invoked
    expect(db.sql.query).toHaveBeenCalled();
  });
});
