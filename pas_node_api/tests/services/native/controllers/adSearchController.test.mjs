...
// ── Mock NativeSearchQueryBuilder (`new`-invoked, fluent) ──────────────
const builderPath = require.resolve("../../../../src/services/native/builders/NativeSearchQueryBuilder");
const builderCalls = [];
function FakeBuilder(indexName) {
  ...
  for (const k of [
    ...
    "setCategory","setCountry","setState","setCity","setAdType","setTargetKeyword","setTags",
    "setLangDetect","setNetwork","setAdPosition","setAdSubPosition","setGender",
...
const parseSort = vi.fn(() => ({ field: "native_ad.last_seen", order: "desc" }));
...
const { searchAds } = require(
  "../../../../src/services/native/controllers/adSearchController"
);
...
beforeEach(() => {
  ...
  parseSort.mockClear().mockImplementation(() => ({ field: "native_ad.last_seen", order: "desc" }));
...
describe("services/native/controllers/adSearchController > validation routing", () => {
...
describe("services/native/controllers/adSearchController > searchFavoriteAds", () => {
  ...
  it("with db.elastic: enrichAndFilterRows drops IMAGE without PowerAdspy and keeps the rest", async () => {
    ...
    const db = {
      ...
      elastic: {
        indexName: "native_search_mix",
        search: vi.fn(async () => ({
          hits: { hits: [
            { _source: { "native_ad.id": 1, "native_ad.nas_url": "https://x/PowerAdspy/img.png" } },
            { _source: { "native_ad.id": 2 } },
            { _source: { "native_ad.id": 3 } },
          ]},
        })),
      },
    };
  ...
  it("enrichAndFilterRows: ES returns body.hits shape (line 122 right operand)", async () => {
    let sqlCall = 0;
    const db = {
      sql: { query: vi.fn(async () => {
        sqlCall++;
...
describe("services/native/controllers/adSearchController > searchHiddenAds", () => {
  it("503 when sql missing", async () => {
...
describe("services/native/controllers/adSearchController > regular searchAds", () => {
  ...
  it("happy path: ES hits → SQL enrich → cleanAdsData", async () => {
    const esHits = [
      { _source: { "native_ad.id": 1, new_nas_image_url: "https://x/nas.png", "native_ad.days_running": 7,
                   "native_ad_url.url_destination": "ud",
                   "native_ad_outgoing_links.source_url": "src" } },
    ];
    const db = {
      elastic: { indexName: "native_search_mix", search: vi.fn(async () => mkEsHits(esHits)) },
  ...
  it("ES uses body.hits fallback + total as number", async () => {
    const db = {
      elastic: { search: vi.fn(async () => ({ body: { hits: { hits: [{ _source: { "native_ad.id": 1 } }], total: 1 } } })) },
    };
    const out = await searchAds({ body: { user_id: "u" }, query: {} }, db, fakeLogger);
    expect(out.total).toBe(1);
  });

  it("SQL fetch failure falls back to mapEsSourceToAd", async () => {
    const esHits = [{ _id: "1", _source: {
      "native_ad.id": 1, "native_ad.type": "TEXT",
      "native_ad_variants.title": "Title", "new_nas_image_url": "https://x/nas.png",
    }}];
    const db = {
    ...
    const out = await searchAds({ body: { user_id: "u" }, query: {} }, db, fakeLogger);
    expect(out.code).toBe(200);
    expect(out.data[0].ad_title).toBe("Title");
    expect(out.data[0].image_video_url).toBe("https://x/nas.png");
    expect(fakeLogger.warn).toHaveBeenCalled();
  });

  it("no db.sql → uses mapEsSourceToAd directly", async () => {
    const esHits = [{ _source: {
      "native_ad.id": 1, "native_ad_variants.title": "Plain Title",
    }}];
    const db = { elastic: { search: vi.fn(async () => mkEsHits(esHits)) } };
    const out = await searchAds({ body: { user_id: "u" }, query: {} }, db, fakeLogger);
    expect(out.data[0].ad_title).toBe("Plain Title");
  });
  ...
  it("invokes builder setters for filter params", async () => {
    ...
    await searchAds({
      body: {
        ...
        type: "IMAGE", call_to_action: "Buy", target_keywords: "kw", tags: "tag1", lang: "en",
        category: "auto", nativeNetwork: "outbrain",
  ...
  it("ES hit without native_ad.id falls back to _id key", async () => {
    const esHits = [{ _id: "alt-id-7", _source: { /* no native_ad.id */ foo: "bar" } }];
    const db = {
  ...
  it("SQL row without matching ES hit returns row unchanged (esHit missing branch)", async () => {
    const esHits = [{ _source: { "native_ad.id": 999 } }];
    const db = {
      elastic: { search: vi.fn(async () => ({ hits: { hits: esHits, total: { value: 1 } } })) },
  ...
  it("ES src missing new_nas_image_url + days_running (false-side branches)", async () => {
    const esHits = [{ _source: { "native_ad.id": 1 /* no new_nas_image_url, no days_running */ } }];
    const db = {
      elastic: { search: vi.fn(async () => ({ hits: { hits: esHits, total: { value: 1 } } })) },
      sql: { query: vi.fn(async () => [{ ad_id: 1, type: "TEXT" }]) },
    };
  ...
  it("ES overlay: src['native_ad.nas_url'] takes precedence over new_nas_image_url (line 416)", async () => {
    const esHits = [{
      _source: {
        "native_ad.id": 1,
        "native_ad.nas_url": "https://x/from-nas-url.png",
        new_nas_image_url: "https://x/from-new-nas.png",
      },
    }];
    const db = {
      elastic: { search: vi.fn(async () => ({ hits: { hits: esHits, total: { value: 1 } } })) },
      sql: { query: vi.fn(async () => [{ ad_id: 1, type: "IMAGE" }]) },
    };
    const out = await searchAds({ body: { user_id: "u" }, query: {} }, db, fakeLogger);
    expect(out.data[0].image_video_url).toBe("https://x/from-nas-url.png");
  });
  it("dedupeRows drops duplicate ad_id rows (line 102)", async () => {
    let call = 0;
    const db = {
      sql: { query: vi.fn(async () => {
        call++;
      ...
      elastic: { indexName: "native", search: vi.fn(async () => ({ hits: { hits: [] } })) },
    };
    const out = await searchAds({ body: { user_id: "u", favorite: "true" }, query: {} }, db, fakeLogger);
