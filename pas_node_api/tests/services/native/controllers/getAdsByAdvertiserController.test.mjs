...
require.cache[networksPath] = {
  ...
  exports: { native: { database: { elastic: { index: "li_idx" } } } },
};
...
const adSearchPath = require.resolve("../../../../src/services/native/controllers/adSearchController");
require.cache[adSearchPath] = {
  id: adSearchPath, filename: adSearchPath, loaded: true,
  exports: { AD_DETAIL_SELECT: "*", AD_DETAIL_JOINS: "FROM native_ad" },
};

const { getAdsByAdvertiser } = require("../../../../src/services/native/controllers/getAdsByAdvertiserController");
...
describe("services/native/getAdsByAdvertiser", () => {
  ...
  it("Native-specific fallback: hit has 'native_ad.nas_url' but no new_nas_image_url (line 31 else-if)", async () => {
    const db = mockDb({
      rows: [{ id: 5 }],
      esResult: { hits: { hits: [{ _source: { "native_ad.nas_url": "http://nas" } }] } },
    });
    const out = await getAdsByAdvertiser({ body: { ad_id: 1 } }, db, fakeLogger);
    expect(out.data[0].image_video_url).toBe("http://nas");
  });
  ...
  it("ES throws → warn logged, flow continues", async () => {
    ...
    const out = await getAdsByAdvertiser({ body: { ad_id: 1 } }, db, fakeLogger);
    expect(out.code).toBe(200);
    expect(fakeLogger.warn).toHaveBeenCalledWith("ES error in Native search", { ad_id: 5 });
  });
  ...
  it("ES index falls back to NATIVE_ELASTIC_INDEX env var (line 26 second operand)", async () => {
    const origExports = require.cache[networksPath].exports;
    require.cache[networksPath].exports = { native: { database: { elastic: {} } } };
    delete require.cache[require.resolve("../../../../src/services/native/controllers/getAdsByAdvertiserController")];
    const reloaded = require("../../../../src/services/native/controllers/getAdsByAdvertiserController");
    const origEnv = process.env.NATIVE_ELASTIC_INDEX;
    process.env.NATIVE_ELASTIC_INDEX = "env_native_idx";
    try {
      ...
      expect(esSearch.mock.calls[0][0].index).toBe("env_native_idx");
    } finally {
      if (origEnv !== undefined) process.env.NATIVE_ELASTIC_INDEX = origEnv;
      else delete process.env.NATIVE_ELASTIC_INDEX;
      require.cache[networksPath].exports = origExports;
    }
  });

  it("ES index falls back to 'native_ads_data' when both config and env missing (line 26 third operand)", async () => {
    const origExports = require.cache[networksPath].exports;
    require.cache[networksPath].exports = { native: { database: { elastic: {} } } };
    delete require.cache[require.resolve("../../../../src/services/native/controllers/getAdsByAdvertiserController")];
    const reloaded = require("../../../../src/services/native/controllers/getAdsByAdvertiserController");
    const origEnv = process.env.NATIVE_ELASTIC_INDEX;
    delete process.env.NATIVE_ELASTIC_INDEX;
    try {
      const esSearch = vi.fn(async () => ({ body: { hits: { hits: [] } } }));
      const db = { sql: { query: vi.fn(async () => [[{ id: 5 }]]) }, elastic: { search: esSearch } };
      await reloaded.getAdsByAdvertiser({ body: { ad_id: 1 } }, db, fakeLogger);
      expect(esSearch.mock.calls[0][0].index).toBe("native_ads_data");
    } finally {
      if (origEnv !== undefined) process.env.NATIVE_ELASTIC_INDEX = origEnv;
      require.cache[networksPath].exports = origExports;
    }
