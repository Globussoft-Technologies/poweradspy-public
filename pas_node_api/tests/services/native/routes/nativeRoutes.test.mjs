...
function mkCtrlMock(filename, exportName) {
  const p = require.resolve(`../../../../src/services/native/controllers/${filename}`);
...
const insightsExports = mkCtrlMock("adInsightsController", [
  "getNativeAdCountry", "getTargetSite", "getAdNetwork", "getRedirect",
  "getRedirectOutgoingUrls", "getAdvertiserCountryData", "getAdvertiserInsightsByDateRange",
]);
...
const { createNativeRoutes } = require(
  "../../../../src/services/native/routes/nativeRoutes"
);
...
describe("nativeRoutes > registration", () => {
  it("registers expected routes", () => {
    const router = createNativeRoutes(svc);
    for (const p of [
      "/ads/search", "/ads/detail", "/ads/getAdDetails",
      "/ads/getNativeAdCountry", "/ads/getTargetSite", "/ads/getAdNetwork",
      "/ads/getRedirect", "/ads/getRedirectOutgoingUrls",
...
describe("nativeRoutes > ResponseFormatter paths", () => {
  it("/ads/search 200", async () => {
    searchAds.mockResolvedValue({ code: 200, data: [], total: 0 });
    await lastHandler(createNativeRoutes(svc), "post", "/ads/search")({ body: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("/ads/search non-200", async () => {
    searchAds.mockResolvedValue({ code: 503 });
    const res = mkRes();
    await lastHandler(createNativeRoutes(svc), "post", "/ads/search")({ body: {} }, res);
    expect(res.statusCode).toBe(503);
  });
  it("/ads/count 200", async () => {
    getAdsCount.mockResolvedValue({ code: 200, data: { c: 1 } });
    await lastHandler(createNativeRoutes(svc), "get", "/ads/count")({ query: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("/ads/count non-200", async () => {
    getAdsCount.mockResolvedValue({ code: 500 });
    const res = mkRes();
    await lastHandler(createNativeRoutes(svc), "get", "/ads/count")({ query: {} }, res);
    expect(res.statusCode).toBe(500);
  });
});

describe("nativeRoutes > pass-through delegates", () => {
  const direct = [
    ["/ads/detail", getAdDetails],
    ["/ads/getAdDetails", getAdDetails],
    ["/ads/getNativeAdCountry", insightsExports.getNativeAdCountry],
    ["/ads/getTargetSite", insightsExports.getTargetSite],
    ["/ads/getAdNetwork", insightsExports.getAdNetwork],
    ["/ads/getRedirect", insightsExports.getRedirect],
  ...
  it.each(direct)("%s 200 path", async (path, ctrl) => {
    ctrl.mockResolvedValue({ code: 200, data: [] });
    const res = mkRes();
    await lastHandler(createNativeRoutes(svc), "post", path)({ body: {} }, res);
  ...
  it("/ads/getAdvertiserInsightsByDateRange null → 400", async () => {
    insightsExports.getAdvertiserInsightsByDateRange.mockResolvedValue(null);
    const res = mkRes();
    await lastHandler(createNativeRoutes(svc), "post", "/ads/getAdvertiserInsightsByDateRange")({ body: {} }, res);
    expect(res.statusCode).toBe(400);
  });
  it("/ads/getAdvertiserInsightsByDateRange 200 passes through", async () => {
    insightsExports.getAdvertiserInsightsByDateRange.mockResolvedValue({ code: 200, data: [] });
    const res = mkRes();
    await lastHandler(createNativeRoutes(svc), "post", "/ads/getAdvertiserInsightsByDateRange")({ body: {} }, res);
    expect(res.statusCode).toBe(200);
  });
  it("/ads/getAdvertiserInsightsByDateRange non-200 result.code propagates (line 149 ternary false branch)", async () => {
    insightsExports.getAdvertiserInsightsByDateRange.mockResolvedValue({ code: 503, message: "es-down" });
    const res = mkRes();
    await lastHandler(createNativeRoutes(svc), "post", "/ads/getAdvertiserInsightsByDateRange")({ body: {} }, res);
    expect(res.statusCode).toBe(503);
  });
