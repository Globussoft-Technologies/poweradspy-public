...
const { getAdsCount } = require("../../../../src/services/quora/controllers/adCountController");

const fakeLogger = { info: vi.fn(), error: vi.fn(), warn: vi.fn() };

beforeEach(() => { fakeLogger.error.mockClear(); });

describe("services/quora/controllers/adCountController > getAdsCount", () => {
  ...
  it("uses QR_ELASTIC_INDEX env when set", async () => {
    process.env.QR_ELASTIC_INDEX = "custom_qr";
    const db = { elastic: { search: vi.fn(async () => ({ hits: { total: 0 } })) } };
    try {
      await getAdsCount({}, db, fakeLogger);
      expect(db.elastic.search.mock.calls[0][0].index).toBe("custom_qr");
    } finally {
      delete process.env.QR_ELASTIC_INDEX;
    }
  });

  it("falls back to 'quora_search_mix' literal when env missing", async () => {
    delete process.env.QR_ELASTIC_INDEX;
    const db = { elastic: { search: vi.fn(async () => ({ hits: { total: 0 } })) } };
    await getAdsCount({}, db, fakeLogger);
    expect(db.elastic.search.mock.calls[0][0].index).toBe("quora_search_mix");
  });
});
