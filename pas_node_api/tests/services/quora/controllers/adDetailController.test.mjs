...
const { getAdDetails } = require(
  "../../../../src/services/quora/controllers/adDetailController"
);
...
describe("services/quora/controllers/adDetailController > validation + SQL paths", () => {
...
describe("services/quora/controllers/adDetailController > ES overlay", () => {
  ...
  it("applies full ES overlay", async () => {
    const esHits = {
      hits: {
        hits: [{
          _source: {
            "quora_translations.es": "Hola",
            "quora_ad_variants.image_brand_logo_exactly": "brand.png",
            "quora_ad_variants.image_object": "obj.png",
            "quora_ad_variants.image_celebrity_exactly": "celeb.png",
            "quora_ad_variants.image_ocr_exactly": "ocr.txt",
            new_nas_image_url: "https://x/nas.png",
            "quora_ad_domain.domain_registered_date": "2020-01-01",
            "quora_ad.days_running": 42,
            "quora.category": "cat",
            "quora.subCategory": "subcat",
            lang_detect: "es",
            "quora_ad_url.url_destination": "url-dest",
            "quora_ad_outgoing_links.source_url": "src-url",
            "quora_ad_outgoing_links.redirect_url": "redir",
            "quora_ad_outgoing_links.final_url": "final",
            "quora_ad_url.url_redirects": "redirects",
            "quora_ad_meta_data.destination_url": "dest",
          },
        }],
      },
    };
    ...
    expect(out.data[0]["quora_translations.es"]).toBe("Hola");
  ...
  it("ES body.hits fallback", async () => {
    const db = {
      sql: { query: vi.fn(adQueryImpl()) },
      elastic: { search: vi.fn(async () => ({ body: { hits: { hits: [{ _source: { "quora.category": "cat" } }] } } })) },
    };
  ...
  it("language='en' → no translation applied", async () => {
    const db = {
      sql: { query: vi.fn(adQueryImpl()) },
      elastic: { search: vi.fn(async () => ({ hits: { hits: [{ _source: { "quora_translations.en": "ignored" } }] } })) },
    };
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, db, fakeLogger);
    expect(out.data[0]["quora_translations.en"]).toBeUndefined();
  });
  ...
  it("language requested but key missing → branch skipped", async () => {
    ...
    expect(out.data[0]["quora_translations.es"]).toBeUndefined();
  });
});

describe("services/quora/controllers/adDetailController > country resolution", () => {
