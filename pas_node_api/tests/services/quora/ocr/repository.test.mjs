...
const repo = require("../../../../src/services/quora/ocr/repository");
...
describe("services/quora/ocr/repository > getImagesUrl", () => {
  it("selects quora_ad_variants joined to quora_ad on type=IMAGE, no image_ocr by default", async () => {
    const exec = mkExec([{ ad_id: 1, image_url: "a.jpg" }]);
    const out = await repo.getImagesUrl(exec, 0, false);
    const { sql, params } = exec.query.mock.calls[0] ? { sql: exec.query.mock.calls[0][0], params: exec.query.mock.calls[0][1] } : {};
    expect(sql).toContain("FROM quora_ad_variants");
    expect(sql).toContain("LEFT JOIN quora_ad ON quora_ad.id = quora_ad_variants.quora_ad_id");
    expect(sql).toContain("quora_ad.type = 'IMAGE'");
    expect(sql).toContain("quora_ad_variants.quora_ad_id AS ad_id");
  ...
  it("includes image_ocr column when withOcr=true and filters by given status", async () => {
    ...
    expect(sql).toContain("quora_ad_variants.image_ocr");
...
describe("services/quora/ocr/repository > updateStatusMultiple", () => {
  it("builds IN(...) placeholders and returns affectedRows", async () => {
    ...
    const sql = exec.query.mock.calls[0][0];
    const params = exec.query.mock.calls[0][1];
    expect(sql).toBe("UPDATE quora_ad_variants SET image_url_status = ? WHERE quora_ad_id IN (?,?,?)");
...
describe("services/quora/ocr/repository > getVariantByAdId", () => {
  it("returns the first row when present", async () => {
    const exec = mkExec([{ id: 1, quora_ad_id: 55 }]);
    const out = await repo.getVariantByAdId(exec, 55);
    expect(exec.query.mock.calls[0][0]).toContain("FROM quora_ad_variants WHERE quora_ad_id = ?");
    expect(out).toEqual({ id: 1, quora_ad_id: 55 });
  });
...
describe("services/quora/ocr/repository > updateVariant", () => {
  it("builds SET clause, skips undefined keys, appends adId, returns affectedRows", async () => {
    ...
    const sql = exec.query.mock.calls[0][0];
    const params = exec.query.mock.calls[0][1];
    expect(sql).toBe("UPDATE quora_ad_variants SET image_object = ?, image_url_status = ? WHERE quora_ad_id = ?");
