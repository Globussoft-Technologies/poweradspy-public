...
const svc = require("../../../../../src/services/quora/ocr/services/getImageUrlService");
...
describe("services/quora/ocr/getImageUrlService > resolveImageUrl", () => {
  ...
  it("resolves a relative path through nasClient", () => {
    expect(svc.resolveImageUrl("/quora/a.jpg")).toBe("https://media.test/quora/a.jpg");
  });
  it("uses only the segment before '||'", () => {
    expect(svc.resolveImageUrl("/quora/a.jpg||/quora/b.jpg")).toBe("https://media.test/quora/a.jpg");
  });
...
describe("services/quora/ocr/getImageUrlService > getImageUrl", () => {
  ...
  it("status 4 → withOcr true, filter 4, resolves urls, flips to in-progress (2)", async () => {
    repo.getImagesUrl.mockResolvedValue([{ ad_id: 10, image_url: "/q/10.jpg", image_ocr: "x" }]);
    ...
    expect(out.data).toEqual([{ ad_id: 10, image_url: "https://media.test/q/10.jpg", image_ocr: "x" }]);
  });
