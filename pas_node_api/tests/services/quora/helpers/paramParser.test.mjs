...
beforeEach(() => {
  ...
  const sutPath = require.resolve("../../../../src/services/quora/helpers/paramParser");
  delete require.cache[sutPath];
  mod = require("../../../../src/services/quora/helpers/paramParser");
});

describe("services/quora/helpers/paramParser > normalizeValue + normalizeParams + ensureArray", () => {
  it("normalizeValue", () => {
...
describe("services/quora/helpers/paramParser > parsePagination", () => {
...
describe("services/quora/helpers/paramParser > parseSort", () => {
  it("sortMap entries", () => {
    expect(mod.parseSort({ newest_sort: "desc" })).toEqual({ field: "quora_ad.id", order: "desc" });
    expect(mod.parseSort({ running_longest_sort: "asc" })).toEqual({ field: "quora_ad.days_running", order: "asc" });
    expect(mod.parseSort({ last_seen_sort: "desc" })).toEqual({ field: "quora_ad.last_seen", order: "desc" });
    expect(mod.parseSort({ hits_sort: "desc" })).toEqual({ field: "quora_ad.hits", order: "desc" });
    expect(mod.parseSort({ domain_sort: "ASC" })).toEqual({ field: "quora_ad_domain.domain_registered_date", order: "asc" });
  });
  it("non-string sort skipped", () => {
    expect(mod.parseSort({ newest_sort: 1 })).toEqual({ field: "quora_ad.last_seen", order: "desc" });
  });
  it("seen_btn_sort array → last_seen desc", () => {
    expect(mod.parseSort({ seen_btn_sort: [1] })).toEqual({ field: "quora_ad.last_seen", order: "desc" });
  });
  it("seen_btn_sort non-array ignored", () => {
    expect(mod.parseSort({ seen_btn_sort: "x" })).toEqual({ field: "quora_ad.last_seen", order: "desc" });
  });
  it.each([
    ["post_date", "quora_ad.post_date"],
    ["last_seen", "quora_ad.last_seen"],
    ["hits", "quora_ad.hits"],
    ["domain_date", "quora_ad_domain.domain_registered_date"],
  ])("order_column %s → %s", (oc, field) => {
    expect(mod.parseSort({ order_column: oc })).toEqual({ field, order: "desc" });
  });
  it("order_column asc respected", () => {
    expect(mod.parseSort({ order_column: "hits", order_by: "asc" })).toEqual({ field: "quora_ad.hits", order: "asc" });
  });
  it("unknown order_column → default", () => {
    expect(mod.parseSort({ order_column: "weird" })).toEqual({ field: "quora_ad.last_seen", order: "desc" });
  });
});

describe("services/quora/helpers/paramParser > withCdn (via cleanAdsData)", () => {
...
describe("services/quora/helpers/paramParser > empty CDN_BASE", () => {
  it("CDN_BASE='' → url unchanged", () => {
    ...
    const sutPath = require.resolve("../../../../src/services/quora/helpers/paramParser");
    delete require.cache[sutPath];
    const noCdn = require("../../../../src/services/quora/helpers/paramParser");
...
describe("services/quora/helpers/paramParser > cleanAdsData", () => {
  ...
  it("malformed JSON kept; plain strings untouched", () => {
