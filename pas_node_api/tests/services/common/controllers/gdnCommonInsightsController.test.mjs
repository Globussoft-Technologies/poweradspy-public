...
const insightsPath = require.resolve("../../../../src/services/gdn/controllers/adInsightsController");
require.cache[insightsPath] = {
  id: insightsPath, filename: insightsPath, loaded: true,
  exports: {
    getGdnAdCountry: vi.fn(),
    getGdnOutgoings: vi.fn(),
    getAdvertiserCountryData: vi.fn(),
  },
};

const detailPath = require.resolve("../../../../src/services/gdn/controllers/adDetailController");
require.cache[detailPath] = {
  id: detailPath, filename: detailPath, loaded: true,
  exports: { getAdDetails: vi.fn() },
};

const { getAdInsights } = require("../../../../src/services/common/controllers/gdnCommonInsightsController");
...
describe("services/common/gdnCommonInsightsController > getAdInsights", () => {
  ...
  it("401 when user_id missing", async () => {
    const res = mockRes();
    await getAdInsights({ body: { gdn_ad_id: "g1" }, query: {} }, res);
    expect(res.status).toHaveBeenCalledWith(401);
  });
  ...
  it("503 when ServiceRegistry has no gdn service", async () => {
    ...
    const res = mockRes();
    await getAdInsights({ body: { gdn_ad_id: "g1", user_id: "u1" }, query: {} }, res);
    expect(res.status).toHaveBeenCalledWith(503);
    expect(res.json).toHaveBeenCalledWith({ code: 503, message: "GDN service not available" });
  });
  ...
  it("delegates to streamInsights with service db + logger", async () => {
    ...
    const req = { body: { gdn_ad_id: "g1", user_id: "u1", language: "en" }, query: {} };
    const res = mockRes();
    await getAdInsights(req, res);
    expect(streamInsights).toHaveBeenCalledWith(
      req, res, expect.any(Array), expect.objectContaining({ gdn_ad_id: "g1", user_id: "u1" }), fakeDb, fakeLog
    );
    ...
    const p = { gdn_ad_id: "g1", user_id: "u1", language: "en" };
    expect(registry[0].payload(p)).toEqual({ ad_id: "g1", user_id: "u1", language: "en" });
    expect(registry[1].payload(p)).toEqual({ gdn_ad_id: "g1", user_id: "u1" });
    expect(registry[2].payload(p)).toEqual({ gdn_ad_id: "g1", user_id: "u1" });
    expect(registry[3].payload(p)).toEqual({ gdn_ad_id: "g1" });
  });

  it("merges req.body and req.query into normalizeParams input", async () => {
    fakeRegistry.getService.mockReturnValueOnce({ db: {}, log: {} });
    const req = { body: { gdn_ad_id: "g1" }, query: { user_id: "u1" } };
    const res = mockRes();
    await getAdInsights(req, res);
    expect(normalizeParams).toHaveBeenCalledWith({ gdn_ad_id: "g1", user_id: "u1" });
