...
const insightsPath = require.resolve("../../../../src/services/linkedin/controllers/adInsightsController");
require.cache[insightsPath] = {
  id: insightsPath, filename: insightsPath, loaded: true,
  exports: {
    getLikeCommentFollowerCount: vi.fn(),
    getLinkedinAdCountry: vi.fn(),
    getLinkedinOutgoings: vi.fn(),
    getAdvertiserLCSData: vi.fn(),
    getAdvertiserCountryData: vi.fn(),
  },
};

const detailPath = require.resolve("../../../../src/services/linkedin/controllers/adDetailController");
require.cache[detailPath] = {
  id: detailPath, filename: detailPath, loaded: true,
  exports: { getAdDetails: vi.fn() },
};

const { getAdInsights } = require("../../../../src/services/common/controllers/linkedinCommonInsightsController");
...
describe("services/common/linkedinCommonInsightsController > getAdInsights", () => {
  ...
  it("401 when user_id missing", async () => {
    const res = mockRes();
    await getAdInsights({ body: { linkedin_ad_id: "l1" }, query: {} }, res);
    expect(res.status).toHaveBeenCalledWith(401);
  });

  it("503 when ServiceRegistry has no linkedin service", async () => {
    fakeRegistry.getService.mockReturnValueOnce(null);
    const res = mockRes();
    await getAdInsights({ body: { linkedin_ad_id: "l1", user_id: "u1" }, query: {} }, res);
    expect(res.status).toHaveBeenCalledWith(503);
    expect(res.json).toHaveBeenCalledWith({ code: 503, message: "LinkedIn service not available" });
  });

  it("delegates to streamInsights with 6-entry registry", async () => {
    fakeRegistry.getService.mockReturnValueOnce({ db: {}, log: {} });
    const req = { body: { linkedin_ad_id: "l1", user_id: "u1", language: "en" }, query: {} };
    const res = mockRes();
    ...
    const p = { linkedin_ad_id: "l1", user_id: "u1", language: "en" };
    expect(registry[0].payload(p)).toEqual({ ad_id: "l1", user_id: "u1", language: "en" });
    expect(registry[1].payload(p)).toEqual({ linkedin_ad_id: "l1", user_id: "u1" });
    expect(registry[2].payload(p)).toEqual({ linkedin_ad_id: "l1", user_id: "u1" });
    expect(registry[3].payload(p)).toEqual({ linkedin_ad_id: "l1", user_id: "u1" });
    expect(registry[4].payload(p)).toEqual({ linkedin_ad_id: "l1", user_id: "u1" });
    expect(registry[5].payload(p)).toEqual({ linkedin_ad_id: "l1" });
  });

  it("merges req.body and req.query into normalizeParams input", async () => {
    fakeRegistry.getService.mockReturnValueOnce({ db: {}, log: {} });
    const req = { body: { linkedin_ad_id: "l1" }, query: { user_id: "u1" } };
    const res = mockRes();
    await getAdInsights(req, res);
    expect(normalizeParams).toHaveBeenCalledWith({ linkedin_ad_id: "l1", user_id: "u1" });
  });
});
