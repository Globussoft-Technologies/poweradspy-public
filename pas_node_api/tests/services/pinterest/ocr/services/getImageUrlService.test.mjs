...
const svc = require("../../../../../src/services/pinterest/ocr/services/getImageUrlService");
...
describe("services/pinterest/ocr/getImageUrlService > resolveImageUrl", () => {
  ...
  it("resolves a relative path through nasClient", () => {
    expect(svc.resolveImageUrl("/pinterest/a.jpg")).toBe("https://media.test/pinterest/a.jpg");
  });
  it("uses only the segment before '||'", () => {
    expect(svc.resolveImageUrl("/pinterest/a.jpg||/pinterest/b.jpg")).toBe("https://media.test/pinterest/a.jpg");
  });
...
describe("services/pinterest/ocr/getImageUrlService > getImageUrl", () => {
  ...
  it("status 4 → withOcr true, filter 4, resolves urls, flips to in-progress (2)", async () => {
    repo.getImagesUrl.mockResolvedValue([{ ad_id: 10, image_url: "/p/10.jpg", image_ocr: "x" }]);
    ...
    expect(out.data).toEqual([{ ad_id: 10, image_url: "https://media.test/p/10.jpg", image_ocr: "x" }]);
  });

  it("status 0 → withOcr false, filter 0", async () => {
    repo.getImagesUrl.mockResolvedValue([{ ad_id: 5, image_url: "/p/5.jpg" }]);
    repo.updateStatusMultiple.mockResolvedValue(1);
  ...
  it("any non-4 status maps to filter 0 / withOcr false", async () => {
    repo.getImagesUrl.mockResolvedValue([{ ad_id: 1, image_url: "/p/1.jpg" }]);
    repo.updateStatusMultiple.mockResolvedValue(1);
