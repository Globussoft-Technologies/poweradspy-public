...
function mkCtrlMock(filename, exportName) {
  const p = require.resolve(`../../../../src/services/pinterest/controllers/${filename}`);
...
const insightsExports = mkCtrlMock("adInsightsController", [
  "getPinterestAdCountry", "getPinterestOutgoings",
...
const { createPinterestRoutes } = require(
  "../../../../src/services/pinterest/routes/pinterestRoutes"
);
...
describe("pinterestRoutes > registration", () => {
  it("registers expected routes", () => {
    const router = createPinterestRoutes(svc);
    for (const p of [
      "/ads/search", "/ads/detail", "/ads/getAdDetails",
      "/ads/getPinterestAdCountry", "/ads/getPinterestOutgoings",
...
describe("pinterestRoutes > ResponseFormatter paths", () => {
  it("/ads/search 200", async () => {
    searchAds.mockResolvedValue({ code: 200, data: [], total: 0 });
    await lastHandler(createPinterestRoutes(svc), "post", "/ads/search")({ body: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("/ads/search non-200", async () => {
    searchAds.mockResolvedValue({ code: 503 });
    const res = mkRes();
    await lastHandler(createPinterestRoutes(svc), "post", "/ads/search")({ body: {} }, res);
    expect(res.statusCode).toBe(503);
  });
  it("/ads/count 200", async () => {
    getAdsCount.mockResolvedValue({ code: 200, data: { c: 1 } });
    await lastHandler(createPinterestRoutes(svc), "get", "/ads/count")({ query: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("/ads/count non-200", async () => {
    getAdsCount.mockResolvedValue({ code: 500 });
    const res = mkRes();
    await lastHandler(createPinterestRoutes(svc), "get", "/ads/count")({ query: {} }, res);
    expect(res.statusCode).toBe(500);
  });
});

describe("pinterestRoutes > pass-through delegates", () => {
  const direct = [
    ["/ads/detail", getAdDetails],
    ["/ads/getAdDetails", getAdDetails],
    ["/ads/getPinterestAdCountry", insightsExports.getPinterestAdCountry],
    ["/ads/getPinterestOutgoings", insightsExports.getPinterestOutgoings],
  ...
  it.each(direct)("%s 200", async (path, ctrl) => {
    ...
    const res = mkRes();
    await lastHandler(createPinterestRoutes(svc), "post", path)({ body: {} }, res);
