...
require.cache[networksPath] = {
  ...
  exports: { pinterest: { database: { elastic: { index: "li_idx" } } } },
};
...
const adSearchPath = require.resolve("../../../../src/services/pinterest/controllers/adSearchController");
require.cache[adSearchPath] = {
  id: adSearchPath, filename: adSearchPath, loaded: true,
  exports: { AD_DETAIL_SELECT: "*", AD_DETAIL_JOINS: "FROM pinterest_ad" },
};

const { getAdsByAdvertiser } = require("../../../../src/services/pinterest/controllers/getAdsByAdvertiserController");
...
describe("services/pinterest/getAdsByAdvertiser", () => {
  ...
  it("ES throws → warn logged, flow continues", async () => {
    ...
    const out = await getAdsByAdvertiser({ body: { ad_id: 1 } }, db, fakeLogger);
    expect(out.code).toBe(200);
    expect(fakeLogger.warn).toHaveBeenCalledWith("ES error in Pinterest search", { ad_id: 5 });
  });
  ...
  it("ES index falls back to PIN_ELASTIC_INDEX env var (line 26 second operand)", async () => {
    const origExports = require.cache[networksPath].exports;
    require.cache[networksPath].exports = { pinterest: { database: { elastic: {} } } };
    delete require.cache[require.resolve("../../../../src/services/pinterest/controllers/getAdsByAdvertiserController")];
    const reloaded = require("../../../../src/services/pinterest/controllers/getAdsByAdvertiserController");
    const origEnv = process.env.PIN_ELASTIC_INDEX;
    process.env.PIN_ELASTIC_INDEX = "env_pin_idx";
    try {
      ...
      expect(esSearch.mock.calls[0][0].index).toBe("env_pin_idx");
    } finally {
      if (origEnv !== undefined) process.env.PIN_ELASTIC_INDEX = origEnv;
      else delete process.env.PIN_ELASTIC_INDEX;
      require.cache[networksPath].exports = origExports;
    }
  });

  it("ES index falls back to 'pinterest_ads_data' when both config and env missing (line 26 third operand)", async () => {
    const origExports = require.cache[networksPath].exports;
    require.cache[networksPath].exports = { pinterest: { database: { elastic: {} } } };
    delete require.cache[require.resolve("../../../../src/services/pinterest/controllers/getAdsByAdvertiserController")];
    const reloaded = require("../../../../src/services/pinterest/controllers/getAdsByAdvertiserController");
    const origEnv = process.env.PIN_ELASTIC_INDEX;
    delete process.env.PIN_ELASTIC_INDEX;
    try {
      const esSearch = vi.fn(async () => ({ body: { hits: { hits: [] } } }));
      const db = { sql: { query: vi.fn(async () => [[{ id: 5 }]]) }, elastic: { search: esSearch } };
      await reloaded.getAdsByAdvertiser({ body: { ad_id: 1 } }, db, fakeLogger);
      expect(esSearch.mock.calls[0][0].index).toBe("pinterest_ads_data");
    } finally {
      if (origEnv !== undefined) process.env.PIN_ELASTIC_INDEX = origEnv;
      require.cache[networksPath].exports = origExports;
    }
