...
// ── Mock PinterestSearchQueryBuilder (`new`-invoked, fluent) ──────────────
const builderPath = require.resolve("../../../../src/services/pinterest/builders/PinterestSearchQueryBuilder");
const builderCalls = [];
function FakeBuilder(indexName) {
  ...
  for (const k of [
    ...
    "setState","setCity","setAdType","setTargetKeyword","setLangDetect","setAdPosition",
    "setAdSubPosition","setLowerAgeSeen","setLastSeen","setPostDate","setDomainDate",
...
const parseSort = vi.fn(() => ({ field: "pinterest_ad.last_seen", order: "desc" }));
...
const { searchAds } = require(
  "../../../../src/services/pinterest/controllers/adSearchController"
);
...
beforeEach(() => {
  ...
  parseSort.mockClear().mockImplementation(() => ({ field: "pinterest_ad.last_seen", order: "desc" }));
...
describe("services/pinterest/controllers/adSearchController > validation routing", () => {
...
describe("services/pinterest/controllers/adSearchController > searchFavoriteAds", () => {
  ...
  it("with db.elastic: enrichAndFilterRows drops IMAGE without PowerAdspy and keeps the rest", async () => {
    ...
    const db = {
      ...
      elastic: {
        indexName: "pinterest_search_mix",
        search: vi.fn(async () => ({
          hits: { hits: [
            { _source: { "pinterest_ad.id": 1, new_nas_image_url: "https://x/PowerAdspy/img.png" } },
            { _source: { "pinterest_ad.id": 2 } },
            { _source: { "pinterest_ad.id": 3 } },
          ]},
        })),
      },
    };
  ...
  it("enrichAndFilterRows: ES returns body.hits shape (line 60 right operand)", async () => {
    let sqlCall = 0;
    const db = {
      sql: { query: vi.fn(async () => {
        sqlCall++;
  ...
  it("dedupeRows drops duplicate ad_id rows (line 45)", async () => {
    let call = 0;
    const db = {
      sql: { query: vi.fn(async () => {
        call++;
      ...
      elastic: { indexName: "pinterest", search: vi.fn(async () => ({ hits: { hits: [] } })) },
    };
    const out = await searchAds({ body: { user_id: "u", favorite: "true" }, query: {} }, db, fakeLogger);
...
describe("services/pinterest/controllers/adSearchController > searchHiddenAds", () => {
  it("503 when sql missing", async () => {
...
describe("services/pinterest/controllers/adSearchController > regular searchAds", () => {
  ...
  it("happy path: ES hits → SQL enrich → cleanAdsData", async () => {
    const esHits = [
      { _source: { "pinterest_ad.id": 1, new_nas_image_url: "https://x/nas.png", "pinterest_ad.days_running": 7,
                   "pinterest_ad_url.url_destination": "ud",
  ...
  // Language must be ES-only (the `language_id` join on pinterest_ad is often
  // 0/unresolved for API-ingested ads) — must agree with the language FILTER,
  // which only ever matches ES `lang_detect`.
  it("language: ES lang_detect resolves the display language, ignoring any SQL languages join", async () => {
    const esHits = [{ _source: { "pinterest_ad.id": 1, lang_detect: "en" } }];
    const db = {
      elastic: { indexName: "pinterest_search_mix", search: vi.fn(async () => mkEsHits(esHits)) },
  ...
  it("language: no ES lang_detect → null, never the SQL languages join value", async () => {
    const esHits = [{ _source: { "pinterest_ad.id": 1 /* no lang_detect */ } }];
    const db = {
      elastic: { indexName: "pinterest_search_mix", search: vi.fn(async () => mkEsHits(esHits)) },
      sql: { query: vi.fn(async () => [{ ad_id: 1, id: 1, type: "TEXT", language: "French" }]) },
    };
  ...
  it("ES uses body.hits fallback + total as number", async () => {
    const db = {
      elastic: { search: vi.fn(async () => ({ body: { hits: { hits: [{ _source: { "pinterest_ad.id": 1 } }], total: 1 } } })) },
    };
    const out = await searchAds({ body: { user_id: "u" }, query: {} }, db, fakeLogger);
    expect(out.total).toBe(1);
  });
  ...
  it("SQL fetch failure falls back to ES sources", async () => {
    const esHits = [{ _id: "i-1", _source: { "pinterest_ad.id": 1, foo: "bar" } }];
    const db = {
  ...
  it("no db.sql → uses ES sources directly", async () => {
    const esHits = [{ _source: { "pinterest_ad.id": 1, foo: "bar" } }];
  ...
  it("invokes builder setters for filter params", async () => {
    ...
    await searchAds({
      body: {
        ...
        type: "IMAGE", target_keywords: "kw", lang: "en", ad_position: "FEED", ad_sub_position: "TOP",
  ...
  it("ES hit without pinterest_ad.id falls back to _id key", async () => {
    const esHits = [{ _id: "alt-id-7", _source: { /* no pinterest_ad.id */ foo: "bar" } }];
    const db = {
  ...
  it("SQL row without matching ES hit returns row unchanged (esHit missing branch)", async () => {
    const esHits = [{ _source: { "pinterest_ad.id": 999 } }];
    const db = {
      elastic: { search: vi.fn(async () => ({ hits: { hits: esHits, total: { value: 1 } } })) },
  ...
  it("ES src missing new_nas_image_url + days_running (false-side branches)", async () => {
    const esHits = [{ _source: { "pinterest_ad.id": 1 /* no new_nas_image_url, no days_running */ } }];
    const db = {
      elastic: { search: vi.fn(async () => ({ hits: { hits: esHits, total: { value: 1 } } })) },
      sql: { query: vi.fn(async () => [{ ad_id: 1, type: "TEXT" }]) },
    };
