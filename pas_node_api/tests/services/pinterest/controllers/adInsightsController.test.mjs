...
} = require("../../../../src/services/pinterest/controllers/adInsightsController");
...
describe("services/pinterest/controllers/adInsightsController > getPinterestAdCountry", () => {
  it("401 when pinterest_ad_id missing", async () => {
    expect(await getPinterestAdCountry({ body: { user_id: "u" }, query: {} }, {}, fakeLogger))
      .toEqual({ code: 401, message: "Missing parameters: pinterest_ad_id and user_id are required" });
  });
  it("401 when user_id missing", async () => {
    expect(await getPinterestAdCountry({ body: { pinterest_ad_id: "1" }, query: {} }, {}, fakeLogger))
      .toEqual({ code: 401, message: "Missing parameters: pinterest_ad_id and user_id are required" });
  });
  it("503 when db.sql missing", async () => {
    expect(await getPinterestAdCountry(
      { body: { pinterest_ad_id: "1", user_id: "u" }, query: {} }, { sql: null }, fakeLogger
    )).toEqual({ code: 503, message: "SQL connection not available" });
  });
  it("400 when no rows", async () => {
    const db = { sql: { query: vi.fn(async () => []) } };
    expect(await getPinterestAdCountry(
      { body: { pinterest_ad_id: "1", user_id: "u" }, query: {} }, db, fakeLogger
  ...
  it("200 with capitalized country + iso fixup (Czechia → CZ)", async () => {
    ...
    const out = await getPinterestAdCountry(
      { body: { pinterest_ad_id: "1", user_id: "u" }, query: {} }, db, fakeLogger
    );
...
describe("services/pinterest/controllers/adInsightsController > getPinterestOutgoings", () => {
  it("401 when pinterest_ad_id missing", async () => {
    expect(await getPinterestOutgoings({ body: {}, query: {} }, {}, fakeLogger))
      .toEqual({ code: 401, message: "Missing parameters: pinterest_ad_id is required" });
  });
  it("503 when db.sql missing", async () => {
    expect(await getPinterestOutgoings(
      { body: { pinterest_ad_id: "1" }, query: {} }, { sql: null }, fakeLogger
    )).toEqual({ code: 503, message: "SQL connection not available" });
  });
  it("200 with URL rows", async () => {
    const db = { sql: { query: vi.fn(async () => [{ source_url: "a", redirect_url: "b", final_url: "c" }]) } };
    const out = await getPinterestOutgoings(
      { body: { pinterest_ad_id: "1" }, query: {} }, db, fakeLogger
    );
    expect(out).toEqual({ code: 200, data: [{ source_url: "a", redirect_url: "b", final_url: "c" }], message: "Urls found" });
  });
  it("400 when no rows", async () => {
    const db = { sql: { query: vi.fn(async () => []) } };
    expect(await getPinterestOutgoings(
...
describe("services/pinterest/controllers/adInsightsController > getAdvertiserCountryData", () => {
  ...
  it("401 when pinterest_ad_id missing", async () => {
    expect(await getAdvertiserCountryData({ body: {}, query: {} }, {}, fakeLogger))
      .toEqual({ code: 401, message: "Missing pinterest_ad_id", data: null });
  });
  ...
  it("explicit p.year overrides ad_last_seen", async () => {
    ...
    const out = await getAdvertiserCountryData(
      { body: { pinterest_ad_id: "1", year: 2020 }, query: {} }, db, fakeLogger
    );
  ...
  it("200 with aggregated country data (docvalue_fields shape)", async () => {
    const db = mkDb({
      ...
      esHits: [
        { fields: { "pinterest_ad.id": [1], "pinterest_country_only.country.keyword": ["germany", "Czechia"] } },
        { fields: { "pinterest_ad.id": [2], "pinterest_country_only.country.keyword": ["germany"] } },
      ],
      countryRows: [
  ...
  it("supports _source shape (date-range variant)", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [
        { _source: { "pinterest_ad.id": 1, "pinterest_country_only.country": "italy" } },
      ],
      countryRows: [],
    });
    const out = await getAdvertiserCountryData(
  ...
  it("non-array country normalized to single-element array", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [{ fields: { "pinterest_ad.id": [1], "pinterest_country_only.country.keyword": "japan" } }],
    });
    const out = await getAdvertiserCountryData(
      { body: { pinterest_ad_id: "1" }, query: {} }, db, fakeLogger
    );
  ...
  it("hit missing adId is skipped", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [
        { fields: { "pinterest_country_only.country.keyword": ["x"] } }, // no ad id
        { fields: { "pinterest_ad.id": [1], "pinterest_country_only.country.keyword": ["spain"] } },
      ],
    });
    const out = await getAdvertiserCountryData(
      { body: { pinterest_ad_id: "1" }, query: {} }, db, fakeLogger
  ...
  it("ES returns body.hits shape → fallback fires (line 253 right operand)", async () => {
    let sqlCall = 0;
    const db = {
      ...
      elastic: {
        search: vi.fn(async (params) => {
          ...
          return { body: { hits: { hits: [
            { fields: { "pinterest_ad.id": [1], "pinterest_country_only.country.keyword": ["germany"] } },
          ]}}};
        }),
      },
    };
  ...
  it("batchCountryLookup SQL throw → empty isoMap, country names used as-is", async () => {
    ...
    const db = {
      ...
      elastic: { search: vi.fn(async () => ({ hits: { hits: [
        { fields: { "pinterest_ad.id": [1], "pinterest_country_only.country.keyword": ["france"] } }
      ]}}))},
    };
    const out = await getAdvertiserCountryData(
      { body: { pinterest_ad_id: "1" }, query: {} }, db, fakeLogger
  ...
  it("hit with no countries is skipped", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [
        { fields: { "pinterest_ad.id": [1] /* no country */ } },
        { fields: { "pinterest_ad.id": [2], "pinterest_country_only.country.keyword": ["spain"] } },
      ],
    });
    const out = await getAdvertiserCountryData(
      { body: { pinterest_ad_id: "1" }, query: {} }, db, fakeLogger
  ...
  it("countryMap with falsy country entries skipped", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [
        { fields: { "pinterest_ad.id": [1], "pinterest_country_only.country.keyword": ["", null, "germany"] } },
      ],
    });
    const out = await getAdvertiserCountryData(
      { body: { pinterest_ad_id: "1" }, query: {} }, db, fakeLogger
  ...
  it("aggregateCountryData returns null when no country buckets produced", async () => {
    const db = mkDb({
      metaRow: { post_owner_name: "Brand", post_owner_id: 5, last_seen: "2024-01-01" },
      esHits: [{ fields: { "pinterest_ad.id": [1] /* no country at all */ } }],
    });
    const out = await getAdvertiserCountryData(
      { body: { pinterest_ad_id: "1" }, query: {} }, db, fakeLogger
    );
...
describe("services/pinterest/controllers/adInsightsController > getAdvertiserInsightsByDateRange", () => {
  ...
  it("country type: 200 with aggregated data", async () => {
    ...
    const db = {
      ...
      elastic: { search: vi.fn(async () => ({ hits: { hits: [
        { _source: { "pinterest_ad.id": 1, "pinterest_country_only.country": "germany" } }
      ]}}))},
    };
    const out = await getAdvertiserInsightsByDateRange(
  ...
  it("country type: data null fallback to [] in response", async () => {
    ...
    const db = {
      ...
      elastic: { search: vi.fn(async () => ({ hits: { hits: [
        { _source: { "pinterest_ad.id": 1 /* no country */ } }
      ]}}))},
    };
    const out = await getAdvertiserInsightsByDateRange(
      { body: { post_owner_id: 5, from_date: "2024-01-01", to_date: "2024-12-31" }, query: {} }, db, fakeLogger
  ...
  it("body.hits fallback in ES result", async () => {
    ...
    const db = {
      ...
      elastic: { search: vi.fn(async () => ({
        body: { hits: { hits: [{ _source: { "pinterest_ad.id": 1, "pinterest_country_only.country": "japan" } }] } }
      }))},
    };
    const out = await getAdvertiserInsightsByDateRange(
      { body: { post_owner_id: 5, from_date: "2024-01-01", to_date: "2024-12-31" }, query: {} }, db, fakeLogger
