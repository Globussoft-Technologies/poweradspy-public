...
const getSvcPath = require.resolve("../../../../src/services/pinterest/ocr/services/getImageUrlService");
const getImageUrl = vi.fn();
require.cache[getSvcPath] = { id: getSvcPath, filename: getSvcPath, loaded: true, exports: { getImageUrl } };

const updSvcPath = require.resolve("../../../../src/services/pinterest/ocr/services/updateImageOcrService");
...
const Controller = require("../../../../src/services/pinterest/controllers/pinterestOcrController");
...
describe("pinterestOcrController > getImageUrl", () => {
...
describe("pinterestOcrController > updateImageOcrDetails", () => {
  it("400 body (HTTP 200) when ad_id missing — service not called", async () => {
    const res = mkRes();
    await Controller.updateImageOcrDetails({ body: {} }, res, null, svc);
    expect(res.statusCode).toBe(200);
    expect(res.body.code).toBe(400);
    expect(res.body.message).toBe(JSON.stringify(["The ad_id field is required."]));
  ...
  it("delegates to service and returns its result (HTTP 200)", async () => {
    ...
    const res = mkRes();
    const body = { ad_id: 107168, status: 4, ocr: "x" };
    await Controller.updateImageOcrDetails({ body }, res, null, svc);
