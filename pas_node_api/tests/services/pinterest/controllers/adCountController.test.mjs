...
const { getAdsCount } = require("../../../../src/services/pinterest/controllers/adCountController");

const fakeLogger = { info: vi.fn(), error: vi.fn(), warn: vi.fn() };

beforeEach(() => { fakeLogger.error.mockClear(); });

describe("services/pinterest/controllers/adCountController > getAdsCount", () => {
  ...
  it("uses PIN_ELASTIC_INDEX env when set", async () => {
    process.env.PIN_ELASTIC_INDEX = "custom_pin";
    const db = { elastic: { search: vi.fn(async () => ({ hits: { total: 0 } })) } };
    try {
      await getAdsCount({}, db, fakeLogger);
      expect(db.elastic.search.mock.calls[0][0].index).toBe("custom_pin");
    } finally {
      delete process.env.PIN_ELASTIC_INDEX;
    }
  });

  it("falls back to 'pinterest_search_mix' literal when env missing", async () => {
    delete process.env.PIN_ELASTIC_INDEX;
    const db = { elastic: { search: vi.fn(async () => ({ hits: { total: 0 } })) } };
    await getAdsCount({}, db, fakeLogger);
    expect(db.elastic.search.mock.calls[0][0].index).toBe("pinterest_search_mix");
  });
});
