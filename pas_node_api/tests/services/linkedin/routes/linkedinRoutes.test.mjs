...
function mkCtrlMock(filename, exportName) {
  const p = require.resolve(`../../../../src/services/linkedin/controllers/${filename}`);
...
const insightsExports = mkCtrlMock("adInsightsController", [
  "getLikeCommentFollowerCount", "getLinkedinAdCountry", "getLinkedinOutgoings",
...
const { createLinkedinRoutes } = require(
  "../../../../src/services/linkedin/routes/linkedinRoutes"
);
...
describe("linkedinRoutes > registration", () => {
  it("registers expected routes", () => {
    const router = createLinkedinRoutes(svc);
    for (const p of [
      "/ads/search", "/ads/detail", "/ads/getAdDetails",
      "/ads/getLikeCommentFollowerCount", "/ads/getLinkedinAdCountry",
      "/ads/getLinkedinOutgoings", "/ads/getAdvertiserLCSData",
...
describe("linkedinRoutes > /ads/search", () => {
  it("200 → success", async () => {
    searchAds.mockResolvedValue({ code: 200, data: [], total: 0 });
    await lastHandler(createLinkedinRoutes(svc), "post", "/ads/search")({ body: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("non-200 passes through", async () => {
    searchAds.mockResolvedValue({ code: 503 });
    const res = mkRes();
    await lastHandler(createLinkedinRoutes(svc), "post", "/ads/search")({ body: {} }, res);
    expect(res.statusCode).toBe(503);
  });
});

describe("linkedinRoutes > /ads/count (GET)", () => {
  it("200 → success", async () => {
    getAdsCount.mockResolvedValue({ code: 200, data: { count: 1 } });
    await lastHandler(createLinkedinRoutes(svc), "get", "/ads/count")({ query: {} }, mkRes());
    expect(ResponseFormatter.success).toHaveBeenCalled();
  });
  it("non-200 passes through", async () => {
    getAdsCount.mockResolvedValue({ code: 500 });
    const res = mkRes();
    await lastHandler(createLinkedinRoutes(svc), "get", "/ads/count")({ query: {} }, res);
    expect(res.statusCode).toBe(500);
  });
});

describe("linkedinRoutes > /ads/detail (passes through, no ResponseFormatter)", () => {
  it("200 path", async () => {
    getAdDetails.mockResolvedValue({ code: 200, data: [{}] });
    const res = mkRes();
    await lastHandler(createLinkedinRoutes(svc), "post", "/ads/detail")({ body: {} }, res);
...
describe("linkedinRoutes > pass-through delegates", () => {
  const direct = [
    ["/ads/getAdDetails", getAdDetails],
    ["/ads/getLikeCommentFollowerCount", insightsExports.getLikeCommentFollowerCount],
    ["/ads/getLinkedinAdCountry", insightsExports.getLinkedinAdCountry],
    ["/ads/getLinkedinOutgoings", insightsExports.getLinkedinOutgoings],
  ...
  it.each(direct)("%s 200 path", async (path, ctrl) => {
    ...
    const res = mkRes();
    await lastHandler(createLinkedinRoutes(svc), "post", path)({ body: {} }, res);
    expect(res.statusCode).toBe(200);
  });
  ...
  it.each(nullGuard)("%s non-200 result.code propagates (line 120/131/142 ternary false branch)", async (path, ctrl) => {
