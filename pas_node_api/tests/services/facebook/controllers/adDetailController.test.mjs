...
const { getAdDetails } = require(
  "../../../../src/services/facebook/controllers/adDetailController"
);
...
describe("services/facebook/controllers/adDetailController > validation + SQL paths", () => {
...
describe("services/facebook/controllers/adDetailController > ad_status (computeAdStatus)", () => {
  function mkDb(adRow) {
    let call = 0;
  ...
  it("Active when last_seen within 15 days", async () => {
    ...
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, mkDb({ id: 1, last_seen: recent }), fakeLogger);
    expect(out.data[0].ad_status).toBe("Active");
  });
  it("Inactive when last_seen > 15 days ago", async () => {
    const old = new Date(Date.now() - 30 * 86400000).toISOString();
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, mkDb({ id: 1, last_seen: old }), fakeLogger);
    expect(out.data[0].ad_status).toBe("Inactive");
  });
  it("Inactive when last_seen null", async () => {
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, mkDb({ id: 1, last_seen: null }), fakeLogger);
    expect(out.data[0].ad_status).toBe("Inactive");
  });
});

describe("services/facebook/controllers/adDetailController > call_to_action capitalization", () => {
...
describe("services/facebook/controllers/adDetailController > ES overlay", () => {
  ...
  it("applies full ES overlay", async () => {
    const esHits = { hits: { hits: [{ _source: {
      "facebook_translations.es": "Hola",
      "facebook_ad.likes": 100, "facebook_ad.shares": 5, "facebook_ad.comments": 10,
      "facebook_ad.impression": 1000, "facebook_ad.popularity": 0.9,
      "facebook.averagebudget": 500,
      "facebook_ad_variants.image_brand_logo_exactly": "brand.png",
      "facebook_ad_variants.image_object": "obj.png",
      "facebook_ad_variants.image_celebrity_exactly": "celeb.png",
      "facebook_ad_variants.image_ocr_exactly": "ocr.txt",
      new_nas_image_url: "https://x/nas.png",
      nas_video_url: "https://x/v.mp4",
      "facebook_ad_domains.domain_registered_date": "2020-01-01",
      "facebook_ad.days_running": 42,
      "facebook_ad_url.url_destination": "url-dest",
      "facebook_ad_outgoing_links.source_url": "src-url",
      "facebook_ad_outgoing_links.redirect_url": "redir",
      "facebook_ad_outgoing_links.final_url": "final",
      "facebook_ad_url.url_redirects": "redirects",
      "facebook_ad_meta_data.destination_url": "dest",
      "facebook.category": "cat",
      "facebook.subCategory": "subcat",
    ...
    expect(out.data[0]["facebook_translations.es"]).toBe("Hola");
  ...
  it("ES body.hits fallback", async () => {
    const db = {
      sql: { query: vi.fn(adQueryImpl()) },
      elastic: { search: vi.fn(async () => ({ body: { hits: { hits: [{ _source: { "facebook.category": "cat" } }] } } })) },
    };
  ...
  it("language='en' → no translation applied", async () => {
    const db = {
      sql: { query: vi.fn(adQueryImpl()) },
      elastic: { search: vi.fn(async () => ({ hits: { hits: [{ _source: { "facebook_translations.en": "ignored" } }] } })) },
    };
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, db, fakeLogger);
    expect(out.data[0]["facebook_translations.en"]).toBeUndefined();
  });
  ...
  it("language requested but key missing → branch skipped", async () => {
    ...
    expect(out.data[0]["facebook_translations.es"]).toBeUndefined();
  });

  it("platform === 15 → LCS skipped", async () => {
    const db = {
      sql: { query: vi.fn(adQueryImpl([{ id: 1, last_seen: null, likes: 1 }])) },
      elastic: { search: vi.fn(async () => ({ hits: { hits: [{ _source: { "facebook_ad.platform": 15, "facebook_ad.likes": 999 } }] } })) },
    };
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, db, fakeLogger);
...
describe("services/facebook/controllers/adDetailController > country resolution", () => {
  it("country_iso applies fixCountryIso (Czechia)", async () => {
    let call = 0;
    const db = { sql: { query: vi.fn(async () => {
      call++;
      if (call === 1) return [{ id: 1, last_seen: null }];
      if (call === 2) return [{ country: "Czechia", facebook_ad_id: 1, iso: null }];
      return [];
    })}};
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, db, fakeLogger);
    expect(out.data[0].country_iso[0]).toEqual({ country: "Czechia", facebook_ad_id: 1, iso: "CZ" });
  });
  it("Russia → RU fixup", async () => {
    ...
    const db = { sql: { query: vi.fn(async () => {
      call++;
      if (call === 1) return [{ id: 1, last_seen: null }];
      if (call === 2) return [{ country: "Russia", facebook_ad_id: 1, iso: null }];
      return [];
    })}};
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, db, fakeLogger);
    expect(out.data[0].country_iso[0].iso).toBe("RU");
  });
  it("DR Congo → CD fixup", async () => {
    ...
    const db = { sql: { query: vi.fn(async () => {
      call++;
      if (call === 1) return [{ id: 1, last_seen: null }];
      if (call === 2) return [{ country: "DR Congo", facebook_ad_id: 1, iso: "XX" }];
      return [];
    })}};
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, db, fakeLogger);
    expect(out.data[0].country_iso[0].iso).toBe("CD");
  });
  it("congo with iso='null' string → CD fixup", async () => {
    ...
    const db = { sql: { query: vi.fn(async () => {
      call++;
      if (call === 1) return [{ id: 1, last_seen: null }];
      if (call === 2) return [{ country: "Republic of Congo", facebook_ad_id: 1, iso: "null" }];
      return [];
    })}};
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, db, fakeLogger);
    expect(out.data[0].country_iso[0].iso).toBe("CD");
  });
  it("country null is preserved (no title-case)", async () => {
    let call = 0;
    const db = { sql: { query: vi.fn(async () => {
      call++;
      if (call === 1) return [{ id: 1, last_seen: null }];
      if (call === 2) return [{ country: null, facebook_ad_id: 1, iso: "US" }];
      return [];
    })}};
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, db, fakeLogger);
    expect(out.data[0].country_iso[0].country).toBeNull();
  });
  ...
  it("country_iso returns null → empty array", async () => {
    let call = 0;
    const db = { sql: { query: vi.fn(async () => {
      call++;
      if (call === 1) return [{ id: 1, last_seen: null }];
...
describe("services/facebook/controllers/adDetailController > built-with API", () => {
  function mkDb({ adRow }) {
    let call = 0;
    return { sql: { query: vi.fn(async () => {
      call++;
  ...
  it("default 501 when no domain", async () => {
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, mkDb({ adRow: { id: 1, last_seen: null } }), fakeLogger);
    expect(out.builtwithStatusCode).toBe(501);
  });
  it("default 501 when domain='null' string", async () => {
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, mkDb({ adRow: { id: 1, last_seen: null, domain: "null" } }), fakeLogger);
    expect(out.builtwithStatusCode).toBe(501);
  });
  it("default 501 when no API_URL_BUILTWITH env", async () => {
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, mkDb({ adRow: { id: 1, last_seen: null, domain: "x.com" } }), fakeLogger);
    expect(out.builtwithStatusCode).toBe(501);
  });
  it("fetches built-with status when URL set and domain present", async () => {
    process.env.API_URL_BUILTWITH = "http://bw.local";
    const mockFetch = vi.fn(async () => ({ json: async () => ({ code: 200 }) }));
    vi.stubGlobal("fetch", mockFetch);
    const out = await getAdDetails({ body: { ad_id: "1" }, query: {} }, mkDb({ adRow: { id: 1, last_seen: null, domain: "x.com" } }), fakeLogger);
    expect(mockFetch).toHaveBeenCalled();
    expect(out.builtwithStatusCode).toBe(200);
  });
  it("built-with API returns no code → defaults to 501", async () => {
    process.env.API_URL_BUILTWITH = "http://bw.local";
