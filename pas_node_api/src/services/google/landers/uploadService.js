...
/**
 * Google landers — upload_gtext_blackhat (BlackhatController@uploadBlackhatContent).
 *
 ...
 * NOTE (vs facebook): gtext returns the screenshot under `image_path` and the zip
 * under `html_path` (separate fields).
 */
  ...
  try {
    if (media || zip) {
      ...
      if (media) {
        ...
        if (folder) {
          const baseName = `${gtextId}_${country}_${status}_${ts}_media`;
          response.image_path = await storeInNas(folder, media.path, gtextId, NETWORK, baseName);
        }
      }
      ...
      if (zip) {
        ...
        if (folder) {
          const baseName = `${gtextId}_${country}_${status}_${ts}_zip`;
          response.html_path = await storeInNas(folder, zip.path, gtextId, NETWORK, baseName);
        }
      }
