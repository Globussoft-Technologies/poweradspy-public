...
/**
 * YouTube delete pipeline — port of YoutubeAdController@deleteads (api_youtube ~4189).
 * Cascade-delete youtube_* children + youtube_ad (one transaction), then remove the doc
 * from Elasticsearch `youtube_ads_data`.
 *
 * The ES _id IS the internal youtube_ad.id (PHP indexes with id = youtube_ad.id), so delete
 * addresses ES by _id = internalId directly. (PHP's delete used a broken `ad_id` key + a
 * searchID() ES round-trip — fixed here to a direct delete by id.)
 *
 * Faithful-but-fixed: atomic transaction + structured errors. Accepts `id` (internal, what
 * the PHP delete uses) or `ad_id` (platform — resolved to the internal id).
 */
  ...
  try {
    ...
    if (db.elastic) {
      try {
        ...
        log.warn('youtube ES delete failed (SQL row already removed)', { id: internalId, error: e.message });
      }
    }

    return { code: 200, status: 'ok', message: 'Data is deleted successfully !', data: { id: internalId } };
  } catch (err) {
    log.error('youtube delete pipeline error', { error: err.message, id: internalId });
