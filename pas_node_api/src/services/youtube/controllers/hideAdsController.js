...
/**
 * YouTube hide/favorite ads controller.
 * Uses youtube_hidden_ads table — mirrors FB/IG/GDN hideAdsController structure.
...
async function hideAds(req, db, logger) {
  try {
    ...
    if (parseInt(type, 10) === 1 && post_owner_id) {
      try {
        await db.sql.query(
          'DELETE FROM youtube_hidden_ads WHERE user_id = ? AND post_owner_id = ? AND type = 3',
    ...
    if (parseInt(type, 10) === 2 && ad_id) {
      try {
        await db.sql.query(
          'DELETE FROM youtube_hidden_ads WHERE user_id = ? AND ad_id = ? AND type = 3',
          [user_id, ad_id]
        );
      } catch (err) {
    ...
    const result = await db.sql.query(
      `INSERT INTO youtube_hidden_ads (user_id, post_owner_id, ad_id, type)
       VALUES (?, ?, ?, ?)`,
    ...
    logger.error('Error in YouTube hideAds', { error: err.message });
...
async function getHiddenPostOwners(req, db, logger) {
  try {
    ...
    const rows = await db.sql.query(
      'SELECT post_owner_id, ad_id, type FROM youtube_hidden_ads WHERE user_id = ?',
      [user_id]
    );
    ...
    logger.error('Error in YouTube getHiddenPostOwners', { error: err.message });
...
async function unHide(req, db, logger) {
  try {
    ...
    if (t === 1) {
      ...
      sql    = 'DELETE FROM youtube_hidden_ads WHERE user_id = ? AND post_owner_id = ? AND type = 1';
      params = [user_id, post_owner_id];
    } else if (t === 2 || t === 3) {
      if (!ad_id) return { code: 400, message: 'Missing ad_id for type=2/3' };
      sql    = 'DELETE FROM youtube_hidden_ads WHERE user_id = ? AND ad_id = ? AND type = ?';
    ...
    logger.error('Error in YouTube unHide', { error: err.message });
    return { code: 500, message: 'Error in YouTube unHide', error: err.message };
  }
}
