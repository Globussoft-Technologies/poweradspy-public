...
/**
 * getImageUrlService — fetches Instagram image ads queued for OCR/OCB processing
 * and marks them in-progress (image_url_status = 2).
 *
 * Faithful port of PHP AdDetails@getImageUrls.
 *   status 0 → pending OCB (object / celebrity / brand) jobs
 *   status 4 → pending OCR (text) jobs (also returns the image_ocr column)
 *
 ...
 * env API_URL for "/image/" paths) into the single NAS media base — same
 * approach the native OCR migration took, keeping URL resolution centralized.
 */
