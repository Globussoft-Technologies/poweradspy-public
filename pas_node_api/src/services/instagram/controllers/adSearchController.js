...
const AD_DETAIL_SELECT = `
    ...
    instagram_ad_analytics.initial_url               AS initial_url,
...
async function searchAds(req, db, logger) {
  ...
  if (Array.isArray(p.first_seen_btn_sort) && p.first_seen_btn_sort.length === 2) {
    builder.setFirstSeen({ lower_date: tsToDate(p.first_seen_btn_sort[1], '00:00:00'), upper_date: tsToDate(p.first_seen_btn_sort[0], '23:59:59') });
  }
    ...
    finalAds = finalAds.map(ad => {
      ...
      return {
        ...
        market_platform_urls: {
          ...
          destination_url: src['instagram_ad_meta_data.destination_url']     || null,
          initial_url:     src['instagram_ad_meta_data.initial_url']         || null,
        },
      };
    });
