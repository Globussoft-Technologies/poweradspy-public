 ...
 * scraper back into MySQL (pinterest_ad_variants) and Elasticsearch
 * (pinterest_search_mix).
 *
 ...
 *
 * Follows the native OCR shape: takes `db` + `log`, derives
...
async function updateImageOcrDetails(postData, db, log) {
  ...
  // 3. Mirror the update into Elasticsearch (pinterest_search_mix).
  if (!elastic) {
  ...
  // Locate the ES document by internal pinterest_ad id.
  const search = await elastic.search({
    index: ES_INDEX,
    body: { query: { match: { 'pinterest_ad.id': adId } } },
  });
  ...
  let docValue = {
    'pinterest_ad_variants.image_object': object,
    'pinterest_ad_variants.image_celebrity': celebrity,
    'pinterest_ad_variants.image_brand_logo': brandLogo,
    'pinterest_ad_variants.image_object_ru': object,
    'pinterest_ad_variants.image_object_fr': object,
    'pinterest_ad_variants.image_object_sp': object,
    'pinterest_ad_variants.image_object_exactly': object,
    'pinterest_ad_variants.image_celebrity_ru': celebrity,
    'pinterest_ad_variants.image_celebrity_fr': celebrity,
    'pinterest_ad_variants.image_celebrity_sp': celebrity,
    'pinterest_ad_variants.image_celebrity_exactly': celebrity,
    'pinterest_ad_variants.image_brand_logo_ru': brandLogo,
    'pinterest_ad_variants.image_brand_logo_fr': brandLogo,
    'pinterest_ad_variants.image_brand_logo_sp': brandLogo,
    'pinterest_ad_variants.image_brand_logo_exactly': brandLogo,
  };

  if (status === 4) {
    docValue = {
      ...docValue,
      'pinterest_ad_variants.image_ocr_ru': ocr,
      'pinterest_ad_variants.image_ocr': ocr,
      'pinterest_ad_variants.image_ocr_fr': ocr,
      'pinterest_ad_variants.image_ocr_sp': ocr,
