...
const { pinterest: pinNet } = require('../../../config/networks');
...
async function getAdsByAdvertiser(req, db, logger) {
  try {
    ...
    const sql = `SELECT ${AD_DETAIL_SELECT} ${AD_DETAIL_JOINS} WHERE pinterest_ad.id = ? LIMIT ${takeNum} OFFSET ${offset}`;
      ...
      if (db.elastic) {
        try {
          const esRes = await db.elastic.search({
            index: pinNet?.database?.elastic?.index || process.env.PIN_ELASTIC_INDEX || 'pinterest_ads_data',
            body: { query: { bool: { filter: { term: { "pinterest_ad.id": value.id } } } } }
          });
          const hit = esRes.body?.hits?.hits?.[0]?._source || esRes.hits?.hits?.[0]?._source;
          if (hit?.new_nas_image_url) value.image_video_url = hit.new_nas_image_url;
        } catch (e) { logger.warn("ES error in Pinterest search", { ad_id: value.id }); }
      }
    }

    return { code: 200, data: cleanAdsData(rows), total: rows.length, message: 'Ads fetched successfully' };
  } catch (err) {
    logger.error('Error in Pinterest getAdsByAdvertiser', { error: err.message });
