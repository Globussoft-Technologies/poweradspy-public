...
const CAPABILITY_DEFINITIONS = [
  ...
  ...[
    ['overview', 'Overview & activity', 'Summary totals and ad activity over time.', '/trends/overview', 'Overview and activity chart', ['facebook', 'instagram', 'google', 'youtube', 'linkedin', 'gdn', 'native', 'reddit', 'quora', 'pinterest', 'tiktok']],
    ['regions', 'Regional demand', 'Country and regional demand data.', '/trends/regions', 'Regional demand', ['facebook', 'instagram', 'google', 'youtube', 'linkedin', 'gdn', 'native', 'reddit', 'quora', 'pinterest']],
    ['categories', 'Category analytics', 'Ads per category and rising category data.', '/trends/categories', 'Ads per category and Rising categories', ['facebook', 'instagram', 'google', 'youtube', 'linkedin', 'gdn', 'native', 'reddit', 'quora', 'pinterest']],
    ['top_movers', 'Top movers', 'Top advertisers and calls-to-action ranked by ad volume.', '/trends/top', 'Top movers', ['facebook', 'instagram', 'google', 'youtube', 'linkedin', 'gdn', 'native', 'reddit', 'quora', 'pinterest']],
    ['keywords', 'Top search keywords', 'Most-targeted search keywords where indexed.', '/trends/keywords', 'Top search keywords', ['google', 'pinterest']],
    ['compare', 'Advertiser comparison', 'Search and compare advertisers inside Market Trends.', '/trends/search', 'Advertiser comparison', ['facebook', 'instagram', 'google', 'youtube', 'linkedin', 'gdn', 'native', 'reddit', 'quora', 'pinterest']],
    ['open_ads_library', 'Open results in Ads Library', 'Open a Market Trends advertiser, keyword, CTA, or category in Ads Library.', '/common/ads/search', 'Open in Ads Library', []],
  ].map(([suffix, label, description, apiPath, location, supportedNetworks]) => ({
    id: `intelligence.market_trends.${suffix}`,
    label,
    description,
    ...
    owner: 'intelligence',
    introducedIn: '2026-07',
    parentCapability: 'intelligence.market_trends',
    routes: suffix === 'open_ads_library'
      ? [{ method: 'POST', path: '/api/v1/common/ads/search', condition: 'Market Trends navigation; destination network must also allow ads.search.' }]
      : [{ method: 'GET', path: `/api/v1/intelligence${apiPath}` }],
    frontend: {
      route: '/market-trends',
      location: `Market Trends > ${location}`,
      controlIds: [`market_trends_${suffix}`],
      previewMode: 'structured',
    },
    lockedExperience: { behavior: 'blur_section_and_show_upgrade', message: `Upgrade to unlock ${label}.` },
  })),
  ...
  ...[
    ['browse', 'Keyword table', 'Browse and paginate the keyword database.', 'Keyword Explorer > Keyword table', ['/api/v1/google/keywords/explorer']],
    ['search', 'Keyword search & import', 'Search keywords or import CSV/TXT keyword lists.', 'Keyword Explorer > Search', ['/api/v1/google/keywords/import', '/api/v1/google/keywords/ideas']],
    ['metrics', 'Summary metrics', 'Keyword count, average competition, total ad volume, and trending totals.', 'Keyword Explorer > Summary cards', ['/api/v1/google/keywords/explorer']],
    ['filters', 'Keyword filters', 'Volume, competition, growth, category, include, and exclude filters.', 'Keyword Explorer > Filters', ['/api/v1/google/keywords/explorer']],
    ['keyword', 'Keyword value', 'Keyword column and keyword analytics entry point.', 'Keyword Explorer > Table > Keyword', ['/api/v1/google/keywords/explorer']],
    ['competition', 'Competition value', 'Competition column and summary value.', 'Keyword Explorer > Table > Competition', ['/api/v1/google/keywords/explorer']],
    ['ad_volume', 'Ad Volume value', 'Ad Volume column and summary value.', 'Keyword Explorer > Table > Ad Volume', ['/api/v1/google/keywords/explorer']],
    ['growth', 'Growth value', 'Growth column and trending summary.', 'Keyword Explorer > Table > Growth', ['/api/v1/google/keywords/explorer']],
    ['parent_topic', 'Parent Topic value', 'Parent Topic/category column.', 'Keyword Explorer > Table > Parent Topic', ['/api/v1/google/keywords/explorer']],
    ['first_seen', 'First Seen value', 'First Seen column.', 'Keyword Explorer > Table > First Seen', ['/api/v1/google/keywords/explorer']],
    ['analytics', 'Keyword analytics drawer', 'Open the detailed analytics drawer for a keyword.', 'Keyword Explorer > Keyword analytics', ['/api/v1/google/keywords/insight']],
    ['open_ads_library', 'Open results in Ads Library', 'Open a keyword advertiser in Google Ads Library.', 'Keyword Explorer > Keyword analytics > Open in Ads Library', ['/api/v1/common/ads/search']],
  ].map(([suffix, label, description, location, apiPaths]) => ({
    id: `intelligence.keyword_explorer.${suffix}`,
    label,
    description,
    category: 'Keyword Explorer',
    status: 'active',
    planControlled: true,
    networkAware: true,
    supportedNetworks: ['google'],
    ...
    owner: 'intelligence',
    introducedIn: '2026-07',
    parentCapability: 'intelligence.keyword_explorer',
    routes: apiPaths.map((path) => ({ method: 'POST', path })),
    frontend: {
      route: '/keywords-explorer',
      location,
      controlIds: [`keyword_explorer_${suffix}`],
      previewMode: 'structured',
    },
    lockedExperience: { behavior: 'blur_section_and_show_upgrade', message: `Upgrade to unlock ${label}.` },
  })),
  ...[
    ['activity', 'Ad activity over time', 'Monthly ad and advertiser activity chart.', 'Ad activity over time'],
    ['top_advertisers', 'Top advertisers', 'Advertisers ranked by distinct ads.', 'Top advertisers'],
    ['top_domains', 'Top landing domains', 'Landing domains ranked by ad count.', 'Top landing domains'],
    ['serp_mix', 'SERP slot mix', 'Top-of-page and bottom-of-page sponsored slot share.', 'SERP slot mix'],
    ['live_creatives', 'Live creatives', 'Recent live ad creative samples.', 'Live creatives'],
  ].map(([suffix, label, description, location]) => ({
    id: `intelligence.keyword_explorer.analytics.${suffix}`,
    label,
    description,
    ...
    owner: 'intelligence',
    introducedIn: '2026-07',
    parentCapability: 'intelligence.keyword_explorer.analytics',
    routes: [{ method: 'POST', path: '/api/v1/google/keywords/insight' }],
    frontend: {
      route: '/keywords-explorer',
      location: `Keyword Explorer > Keyword analytics > ${location}`,
      controlIds: [`keyword_analytics_${suffix}`],
      previewMode: 'structured',
    },
    lockedExperience: { behavior: 'blur_section_and_show_upgrade', message: `Upgrade to unlock ${label}.` },
  })),
