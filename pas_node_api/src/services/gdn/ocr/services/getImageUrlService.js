...
/**
 * GDN OCR/OCB — getImageUrl (lease work).
 *
 * Faithful port of api_gdn ApiController@getImageUrl, the handler behind
 * `GET getGDNImageUrl`. Hands out up to 20 IMAGE ads queued for processing, resolves
 * each image_url to an absolute NAS URL, and marks the batch in-progress
 ...
 *
 * Unlike Facebook, the GDN lease has NO 10-day last_seen window. Returns
...
async function leaseImages(db, log, status) {
  ...
  try {
    ...
    log?.error?.('gdn.ocr.getImageUrl failed', { error: e.message });
    return { code: 401, message: 'No More Image are present', data: [], exe_time: exeTime() };
  }
}
