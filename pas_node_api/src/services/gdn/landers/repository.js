...
/**
 * GDN landers — data repository (raw parameterized SQL).
 *
 * Faithful port of the Laravel model methods used by the three GDN PHP endpoints
 * (api_gdn BlackhatController: getGDNAdsWithCounrty / uploadBlackhatContent /
 * inserHtmlContentToDB). One function per DB operation, grouped by table — same
 * shape as the google (gtext) landers repository.
 *
 ...
 *
 * Tables (DB pasdev_gdn):
 *   gdn_ad_meta_data        (PK gdn_ad_id)
 *   gdn_ad_domains
 *   gdn_ad_url
 *   gdn_ad_outgoing_links
 *   gdn_ad_html_lander_content
 *   gdn_ad                  (main ad; PK id, holds domain_id)
 *   gdn_ad_countries_only / gdn_country_only / country_data (lookups)
 */
 ...
 * GROUP BY is valid under MySQL only_full_group_by.
...
async function getDataForLander(exec, redirectStatus) {
  ...
  const sql = `
    SELECT gdn_ad_meta_data.gdn_ad_id AS id,
           ANY_VALUE(gdn_ad_meta_data.destination_url) AS destination_url,
           GROUP_CONCAT(gdn_country_only.country) AS country
      FROM gdn_ad_meta_data
      LEFT JOIN gdn_ad_countries_only
             ON gdn_ad_countries_only.gdn_ad_id = gdn_ad_meta_data.gdn_ad_id
      LEFT JOIN gdn_country_only
             ON gdn_country_only.id = gdn_ad_countries_only.country_only_id
     WHERE gdn_ad_meta_data.redirect_status = ?
       AND gdn_ad_meta_data.destination_url IS NOT NULL
       AND TRIM(gdn_ad_meta_data.destination_url) <> ''
       AND LOWER(TRIM(gdn_ad_meta_data.destination_url)) NOT IN ('null', 'undefined')
     GROUP BY gdn_ad_meta_data.gdn_ad_id
     ORDER BY gdn_ad_meta_data.gdn_ad_id DESC
     LIMIT 50`;
...
async function updateMetaMultiple(exec, adIds, data) {
  ...
  const sql = `UPDATE gdn_ad_meta_data SET ${cols.map((c) => `${c} = ?`).join(', ')} WHERE gdn_ad_id IN (${placeholders})`;
...
async function getMetaDataDetails(exec, adId) {
  const sql = `
    SELECT gdn_ad_id AS id, white_ad_screenshot, png_file, white_ad_lander,
...
/** PHP updateData(): UPDATE ... WHERE gdn_ad_id = ?. Returns affectedRows (0 = no change). */
async function updateMeta(exec, adId, data) {
  const cols = Object.keys(data);
  if (!cols.length) return 0;
  const sql = `UPDATE gdn_ad_meta_data SET ${cols.map((c) => `${c} = ?`).join(', ')} WHERE gdn_ad_id = ?`;
  return affected(await exec.query(sql, [...Object.values(data), adId]));
}

/** PHP GdnAd.updateData(): set domain_id on the main gdn_ad row (WHERE id = ?). */
async function updateMainAdDomainId(exec, adId, domainId) {
  return affected(await exec.query('UPDATE gdn_ad SET domain_id = ? WHERE id = ?', [domainId, adId]));
}

// ── gdn_ad_domains ────────────────────────────────────────────────────────────
// Note: the GDN lander does NOT stamp a dod_date.

async function getDomainId(exec, domain) {
  return rows(await exec.query('SELECT id FROM gdn_ad_domains WHERE domain = ?', [domain]));
}
async function updateDomainRegisterDate(exec, id, date) {
  return affected(await exec.query('UPDATE gdn_ad_domains SET domain_registered_date = ? WHERE id = ?', [date, id]));
}
async function insertDomainName(exec, data) {
  const clean = stripNulls(data);
  const cols = Object.keys(clean);
  const sql = `INSERT INTO gdn_ad_domains (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`;
  // PHP uses ->insert() (bool); we use insertGetId so the main ad's domain_id is the
  // real inserted id (mirrors the google lander; corrects the legacy bool quirk).
...
async function getOutgoingDetails(exec, w) {
  ...
  return rows(await exec.query(sql, [
    w.gdn_ad_id, w.source_url ?? null, w.redirect_url ?? null,
...
async function insertOutgoing(exec, data) {
  const clean = stripNulls(data);
  const cols = Object.keys(clean);
  const sql = `INSERT INTO gdn_ad_outgoing_links (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`;
  return firstId(await exec.query(sql, Object.values(clean)));
}
/**
 * PHP updateOutgoingUrls($where,$data): UPDATE ... SET country_code = ? WHERE gdn_ad_id = ?.
...
async function updateOutgoingCountry(exec, adIdWhere, countryCode) {
  return affected(await exec.query(
    'UPDATE gdn_ad_outgoing_links SET country_code = ? WHERE gdn_ad_id = ?',
...
/** PHP getDestinationDetails(): match on url_type + gdn_ad_id + url + proxy_lander_status. */
  ...
  const sql = `
    SELECT ${cols}
     ...
     WHERE url_type <=> ?
       ...
       AND proxy_lander_status <=> ?`;
  return rows(await exec.query(sql, [
    w.url_type ?? null, w.gdn_ad_id, w.url ?? null, w.proxy_lander_status ?? null,
  ]));
}
async function insertAdUrl(exec, data) {
  const clean = stripNulls(data);
  const cols = Object.keys(clean);
  const sql = `INSERT INTO gdn_ad_url (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`;
  return firstId(await exec.query(sql, Object.values(clean)));
}
/** PHP updateData(): UPDATE gdn_ad_url ... WHERE gdn_ad_id = ?. */
async function updateAdUrl(exec, adId, data) {
  const cols = Object.keys(data);
  if (!cols.length) return 0;
  const sql = `UPDATE gdn_ad_url SET ${cols.map((c) => `${c} = ?`).join(', ')} WHERE gdn_ad_id = ?`;
  return affected(await exec.query(sql, [...Object.values(data), adId]));
}
/** PHP getCountryCode(): every stored country_code for this ad's urls. */
async function getCountryCodes(exec, adId) {
  return rows(await exec.query('SELECT country_code FROM gdn_ad_url WHERE gdn_ad_id = ?', [adId]));
}

// ── gdn_ad_html_lander_content ────────────────────────────────────────────────

async function getHtmlLanderDetails(exec, adId) {
  return rows(await exec.query('SELECT id FROM gdn_ad_html_lander_content WHERE gdn_ad_id = ?', [adId]));
}
async function insertHtmlFile(exec, data) {
  const clean = stripNulls(data);
  const cols = Object.keys(clean);
  const sql = `INSERT INTO gdn_ad_html_lander_content (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`;
  return firstId(await exec.query(sql, Object.values(clean)));
}
async function updateHtmlFile(exec, adId, data) {
  const cols = Object.keys(data);
  if (!cols.length) return 0;
  const sql = `UPDATE gdn_ad_html_lander_content SET ${cols.map((c) => `${c} = ?`).join(', ')} WHERE gdn_ad_id = ?`;
  return affected(await exec.query(sql, [...Object.values(data), adId]));
}
...
/** PHP: nicename for an ISO code (insert_html ES country_code resolution). */
