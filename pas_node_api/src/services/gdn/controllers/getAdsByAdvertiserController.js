...
const { gdn: gdnNet } = require('../../../config/networks');
...
async function getAdsByAdvertiser(req, db, logger) {
  try {
    ...
    const sql = `SELECT ${AD_DETAIL_SELECT} ${AD_DETAIL_JOINS} WHERE gdn_ad.id = ? LIMIT ${takeNum} OFFSET ${offset}`;
      ...
      if (db.elastic) {
        try {
          const esRes = await db.elastic.search({
            index: gdnNet?.database?.elastic?.index || process.env.GDN_ELASTIC_INDEX || 'gdn_ads_data',
            body: { query: { bool: { filter: { term: { "gdn_ad.id": row.id } } } } }
          });
        ...
        } catch (e) { logger.warn("ES error in GDN search", { ad_id: row.id }); }
      }
    }

    return { code: 200, data: cleanAdsData(rows), total: rows.length, message: 'Ads fetched successfully' };
  } catch (err) {
    logger.error('Error in GDN getAdsByAdvertiser', { error: err.message });
