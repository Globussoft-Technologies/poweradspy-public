'use strict';

/**
 * Quora built-with scrape queue controller.
 * Ports ScrappingController@getBuiltWithAds and @updateBuiltWithAds.
 *
 *  GET  /getUrlForBuiltWith → pull up to 100 ads with built_with_status=0
 *  POST /updateBuiltWith    → worker reports results back for one ad
 */

const QUORA_ES_INDEX = 'quora_search_mix';

function normalizePipe(v) {
  if (v == null) return null;
  const s = String(v).trim();
  if (s === '') return null;
  return s.replace(/\|\|/g, '|');
}

function normalizeAffiliate(v) {
  if (v == null || v === '') return null;
  return v;
}

async function getUrlForBuiltWith(req, db, logger) {
  if (!db.sql) {
    return { code: 503, message: 'SQL connection not available', data: null };
  }
  try {
    const rows = await db.sql.query(
      `SELECT quora_ad_id AS id, destination_url
         FROM quora_ad_meta_data
        WHERE built_with_status = 0
        ORDER BY quora_ad_id DESC
        LIMIT 100`
    );

    if (!rows || rows.length === 0) {
      return {
        code: 400,
        message: 'No more ads available for builtwith',
        data: null,
      };
    }

    const ids = rows.map(r => r.id);
    const placeholders = ids.map(() => '?').join(',');
    await db.sql.query(
      `UPDATE quora_ad_meta_data
          SET built_with_status = 2,
              affiliate_status  = 2
        WHERE quora_ad_id IN (${placeholders})`,
      ids
    );

    return { code: 200, message: 'Ads found for builtwith', data: rows };
  } catch (err) {
    logger.error('Error in getBuiltWithAds (quora)', { error: err.message });
    return { code: 500, message: 'Error occured', data: null };
  }
}

async function updateBuiltWith(req, db, logger) {
  if (!db.sql) {
    return { code: 503, message: 'SQL connection not available' };
  }

  const post = { ...req.body, ...req.query };
  if (post.id == null || post.status == null) {
    return { code: 400, message: ['The id field is required.', 'The status field is required.'] };
  }

  const adId = post.id;
  const status = Number(post.status);
  const now = new Date().toISOString().slice(0, 19).replace('T', ' ');

  try {
    // Confirm the row exists before doing any writes (matches PHP behaviour).
    const existing = await db.sql.query(
      'SELECT quora_ad_id FROM quora_ad_meta_data WHERE quora_ad_id = ? LIMIT 1',
      [adId]
    );
    if (!existing || existing.length === 0) {
      return { code: 200, message: 'parameters empty', data: post };
    }

    if (status === 1) {
      const built_with                     = normalizePipe(post.built_with);
      const built_with_analytics_tracking  = normalizePipe(post.built_with_analytics_tracking);
      const built_with_cms                 = normalizePipe(post.built_with_cms);
      const affiliate_data                 = normalizeAffiliate(post.affiliate_data);

      const built_with_status = (built_with || built_with_analytics_tracking || built_with_cms) ? 1 : 3;
      const affiliate_status  = affiliate_data ? 1 : 3;

      const update = await db.sql.query(
        `UPDATE quora_ad_meta_data
            SET built_with                     = ?,
                built_with_analytics_tracking  = ?,
                built_with_cms                 = ?,
                built_with_status              = ?,
                built_with_date                = ?,
                affiliate_data                 = ?,
                affiliate_status               = ?,
                clickbank_processed_date       = ?
          WHERE quora_ad_id = ?`,
        [built_with, built_with_analytics_tracking, built_with_cms, built_with_status, now,
         affiliate_data, affiliate_status, now, adId]
      );
      const affected = typeof update?.affectedRows === 'number' ? update.affectedRows : update;

      if (!affected) {
        return { code: 400, message: 'built with not updated' };
      }

      // Best-effort ES overlay — do not fail the request if ES is down.
      if (db.elastic) {
        try {
          const index = db.elastic.indexName || QUORA_ES_INDEX;
          const search = await db.elastic.search({
            index,
            body: { query: { match: { 'quora_ad.id': Number(adId) } }, size: 1 },
          });
          const hits = search?.hits?.hits || search?.body?.hits?.hits || [];
          if (hits.length === 0) {
            return { code: 400, message: 'ad not found' };
          }
          await db.elastic.update({
            index,
            type: 'doc',
            id: hits[0]._id,
            body: {
              doc: {
                'quora_ad_meta_data.built_with': built_with,
                'quora_ad_meta_data.built_with_analytics_tracking': built_with_analytics_tracking,
                'quora_ad_meta_data.affiliate_data': affiliate_data,
              },
            },
          });
        } catch (esErr) {
          logger.warn('quora ES overlay failed in updateBuiltWith', { error: esErr.message });
        }
      }

      return { code: 200, message: 'built with updated' };
    }

    // status !== 1 → mark row as no-data.
    await db.sql.query(
      `UPDATE quora_ad_meta_data
          SET built_with_status         = 3,
              built_with_date           = ?,
              affiliate_status          = 3,
              clickbank_processed_date  = ?
        WHERE quora_ad_id = ?`,
      [now, now, adId]
    );

    return { code: 200, message: 'built with updated as Null' };
  } catch (err) {
    logger.error('Error in updateBuiltWithAds (quora)', { error: err.message });
    return { code: 400, message: 'Exception Occured' };
  }
}

module.exports = { getUrlForBuiltWith, updateBuiltWith };
