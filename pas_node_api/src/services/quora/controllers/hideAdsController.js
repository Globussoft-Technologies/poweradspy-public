...
/**
 * Quora hide/favorite ads controller.
 * Uses hidden_ads table.
...
async function hideAds(req, db, logger) {
  try {
    ...
    const result = await db.sql.query(
      `INSERT INTO hidden_ads (user_id, post_owner_id, ad_id, type)
       VALUES (?, ?, ?, ?)`,
    ...
    logger.error('Error in Quora hideAds', { error: err.message });
...
async function getHiddenPostOwners(req, db, logger) {
  try {
    ...
    logger.error('Error in Quora getHiddenPostOwners', { error: err.message });
...
async function unHide(req, db, logger) {
  try {
    ...
    logger.error('Error in Quora unHide', { error: err.message });
    return { code: 500, message: 'Error in Quora unHide', error: err.message };
  }
}
