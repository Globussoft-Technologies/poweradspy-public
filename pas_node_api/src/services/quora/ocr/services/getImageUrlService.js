...
/**
 * getImageUrlService — fetches quora image ads queued for OCR/OCB processing
 * and marks them in-progress (image_url_status = 2).
 *
 * Faithful port of PHP UserController@getImageUrls.
 *   status 0 → pending OCB (object / celebrity / brand) jobs
 *   status 4 → pending OCR (text) jobs (also returns image_ocr column)
 *
 * Follows the native ocr / gdn landers shape: takes `db` + `log`, derives `db.sql`,
