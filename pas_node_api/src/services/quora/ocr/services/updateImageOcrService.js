 ...
 * scraper back into MySQL (quora_ad_variants) and Elasticsearch
 * (quora_search_mix).
 *
 ...
 *
 * Follows the native ocr / gdn landers shape: takes `db` + `log`, derives
...
async function updateImageOcrDetails(postData, db, log) {
  ...
  // 3. Mirror the update into Elasticsearch (quora_search_mix).
  if (!elastic) {
  ...
  // Locate the ES document by internal quora_ad id.
  const search = await elastic.search({
    index: ES_INDEX,
    body: { query: { match: { 'quora_ad.id': adId } } },
  });
  ...
  let docValue = {
    'quora_ad_variants.image_object': object,
    'quora_ad_variants.image_celebrity': celebrity,
    'quora_ad_variants.image_brand_logo': brandLogo,
    'quora_ad_variants.image_object_ru': object,
    'quora_ad_variants.image_object_fr': object,
    'quora_ad_variants.image_object_sp': object,
    'quora_ad_variants.image_object_exactly': object,
    'quora_ad_variants.image_celebrity_ru': celebrity,
    'quora_ad_variants.image_celebrity_fr': celebrity,
    'quora_ad_variants.image_celebrity_sp': celebrity,
    'quora_ad_variants.image_celebrity_exactly': celebrity,
    'quora_ad_variants.image_brand_logo_ru': brandLogo,
    'quora_ad_variants.image_brand_logo_fr': brandLogo,
    'quora_ad_variants.image_brand_logo_sp': brandLogo,
    'quora_ad_variants.image_brand_logo_exactly': brandLogo,
  };

  if (status === 4) {
    docValue = {
      ...docValue,
      'quora_ad_variants.image_ocr_ru': ocr,
      'quora_ad_variants.image_ocr': ocr,
      'quora_ad_variants.image_ocr_fr': ocr,
      'quora_ad_variants.image_ocr_sp': ocr,
      'quora_ad_variants.image_ocr_exactly': ocr,
    };
  }
  ...
  // PHP parity: Quora returns "ad not found" (not "Image Object not updated")
  // when the ES update is a no-op.
  return { code: 400, message: 'ad not found' };
}
