...
/**
 * LinkedIn landers — upload_file_to-server (api_linkedin BlackhatController@uploadBlackhatContent).
 * Same shape as youtube/landers/uploadService.js.
 *
...
async function uploadBlackhatContent(req, log) {
  ...
  try {
    if (media || zip) {
      ...
      const linkedinId = req.body?.ad_id;
      ...
      if (media) {
        ...
        if (folder) {
          const baseName = `${linkedinId}_${country}_${status}_${ts}_media`;
          response.image_path = await storeInNas(folder, media.path, linkedinId, NETWORK, baseName);
        }
      }
      ...
      if (zip) {
        ...
        if (folder) {
          const baseName = `${linkedinId}_${country}_${status}_${ts}_zip`;
          response.html_path = await storeInNas(folder, zip.path, linkedinId, NETWORK, baseName);
        }
      }
