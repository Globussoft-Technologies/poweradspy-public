...
const { linkedin: liNet } = require('../../../config/networks');
...
async function getAdsByAdvertiser(req, db, logger) {
  try {
    ...
    const sql = `SELECT ${AD_DETAIL_SELECT} ${AD_DETAIL_JOINS} WHERE linkedin_ad.id = ? LIMIT ${takeNum} OFFSET ${offset}`;
      ...
      if (db.elastic) {
        try {
          const esRes = await db.elastic.search({
            index: liNet?.database?.elastic?.index || process.env.LI_ELASTIC_INDEX || 'linkedin_ads_data',
            body: { query: { bool: { filter: { term: { "ad_id": row.id } } } } }
          });
        ...
        } catch (e) { logger.warn("ES error in LinkedIn search", { ad_id: row.id }); }
      }
    }

    return { code: 200, data: cleanAdsData(rows), total: rows.length, message: 'Ads fetched successfully' };
  } catch (err) {
    logger.error('Error in LinkedIn getAdsByAdvertiser', { error: err.message });
