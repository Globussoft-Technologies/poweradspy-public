...
/**
 * LinkedIn OCR/OCB controller (thin).
 *
 * HTTP-facing layer for the image OCB/OCR pipeline: validates the request and delegates
 * to the two services. Each returns a plain { code, message, ... } object; the route
 * always replies HTTP 200 with that body (PHP contract — the outcome is in `code`).
 * Mirrors the controller→service layering used by the gdn/facebook OCR endpoints.
 */
...
// GET get-linkedin-image-url — `status` accepted in the query string (also body, PHP parity).
...
// POST update-image-info — `ad_id` required.
