...
async function claimOne(col, { type, net, isPriority, owner, today, now, sortDir }) {
  ...
  return doc ? { docId: doc._id, type: doc.type, value: doc.value, network: net, scrapeId, mode } : null;
}
...
// (endTime + status). No adsCount needed. Default status = 'completed'; the scraper may
// send 'no_ads_found' / 'failed'. Works for BOTH daily and priority modes.
async function autoCloseOwnerSessions(col, { owner, net, type, status, today, now }) {
  const allowed = ['completed', 'no_ads_found', 'failed'];
  const finalStatus = allowed.includes(status) ? status : 'completed';
  ...
  // Match docs that have an open session for this owner+network+type, then close it.
...
//
// Body: {
//   type, network,                     // REQUIRED — the work stream (one type+network)
//   priority?: true,                   // priority mode (default daily)
...
async function scraperWork(req, res) {
  try {
    ...
    const body = req.body || {};
    const type = normType(body.type ?? req.query.type);
    if (!type) {
      return res.status(400).json({ code: 400, message: "'type' is required: keyword|advertiser|domain (or 1|2|3)", data: null });
    }
    const { net, error: netError } = resolveClaimNetwork(body.network ?? req.query.network);
    ...
    if (explicitResults.length > 0) {
      for (const r of explicitResults) {
        try {
          ...
          const rnet = resolveClaimNetwork(r.network).net || net; // default to this stream's network
    ...
    for (let i = 0; i < size; i++) {
      const item = await claimOne(col, { type, net, isPriority, owner, today, now, sortDir });
      if (!item) break; // pool exhausted for this type+network
      claimed.push(item);
    }
