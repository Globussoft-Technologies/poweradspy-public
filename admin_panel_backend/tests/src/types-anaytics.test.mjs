...
describe("src/types-anaytics > typesStatsWithFilter", () => {
  it("400 when network missing or unknown", async () => {
    const res = mockRes();
    await typesStatsWithFilter({ body: {} }, res);
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ message: "Please provide valid network" });

    const res2 = mockRes();
    await typesStatsWithFilter({ body: { network: "tiktok" } }, res2);
    expect(res2.status).toHaveBeenCalledWith(400);
  });

  it("no-filter aggregation: GROUP BY type + ORDER BY num DESC", async () => {
    queryDatabaseSpy.mockResolvedValueOnce([
      { type: "image", num: 100 },
      { type: "video", num: 30 },
    ]);
    const res = mockRes();
    await typesStatsWithFilter({ body: { network: "facebook" } }, res);
    const [db_id, , sql, params] = queryDatabaseSpy.mock.calls[0];
    expect(db_id).toBe(0);
    expect(sql).toMatch(/SELECT type, COUNT\(id\) AS num FROM facebook_ad/);
    expect(sql).toMatch(/GROUP BY type ORDER BY num DESC/);
    expect(params).toEqual([]);
    const out = res.json.mock.calls[0][0];
    expect(out.type).toBe("agg");
    expect(out.data).toEqual([
      { category: "image", value: 100 },
      { category: "video", value: 30 },
    ]);
    expect(out.total).toEqual({ value: 130, relation: "eq" });
  });

  it("type only: COUNT query with WHERE type = ?", async () => {
    queryDatabaseSpy.mockResolvedValueOnce([{ cnt: 42 }]);
    const res = mockRes();
    await typesStatsWithFilter(
      { body: { network: "facebook", type: "image" } },
      res,
    );
    const [, , sql, params] = queryDatabaseSpy.mock.calls[0];
    expect(sql).toMatch(/SELECT COUNT\(id\) AS cnt FROM facebook_ad WHERE type = \?/);
  ...
  it("type + range: combined WHERE clause", async () => {
    ...
    const [, , sql, params] = queryDatabaseSpy.mock.calls[0];
    expect(sql).toMatch(/WHERE type = \? AND first_seen BETWEEN \? AND \?/);
    expect(params).toEqual(["image", "2025-01-01 00:00:00", "2025-01-31 23:59:59"]);
    expect(res.json.mock.calls[0][0].total).toBe(6);
  });

  it("type + range + linkedin: uses linkedin_ad table + db_id=2", async () => {
  ...
  it("type path: rows[0].cnt null → total 0", async () => {
    queryDatabaseSpy.mockResolvedValueOnce([{ cnt: null }]);
    const res = mockRes();
    await typesStatsWithFilter({ body: { network: "facebook", type: "image" } }, res);
    expect(res.json.mock.calls[0][0].total).toBe(0);
  });

  it("type path: rows null → total 0", async () => {
    queryDatabaseSpy.mockResolvedValueOnce(null);
    const res = mockRes();
    await typesStatsWithFilter({ body: { network: "facebook", type: "image" } }, res);
    expect(res.json.mock.calls[0][0].total).toBe(0);
  ...
  it.each([
    ...
    queryDatabaseSpy.mockResolvedValueOnce([]);
    const res = mockRes();
    await typesStatsWithFilter({ body: { network } }, res);
    const [db_id, , sql] = queryDatabaseSpy.mock.calls[0];
