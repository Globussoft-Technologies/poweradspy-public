...
// DS Q6: SELECT type, COUNT(id) AS num FROM <net>_ad
//        WHERE first_seen >= yesterday AND first_seen < today GROUP BY type
// All networks follow the standard pattern — no per-network deviations for Q6.
const DB_DATA = {
...
const typesStatsWithFilter = async (req, res) => {
    try {
        const { type, range, network } = req.body;
        ...
        if (type) {
            // Single-type lookup → return a count, matching the prior `responseType: "count"` shape.
            let sql = `SELECT COUNT(id) AS cnt FROM ${mainTable} WHERE type = ?`;
            const params = [type];
            if (hasRange) {
        ...
        // Aggregation by type.
        let sql = `SELECT type, COUNT(id) AS num FROM ${mainTable}`;
        const params = [];
        if (hasRange) {
            sql += ' WHERE first_seen BETWEEN ? AND ?';
            params.push(fromTs, toTs);
        }
        sql += ' GROUP BY type ORDER BY num DESC';

        const rows = await queryDatabase(db_id, index, sql, params);
        const data = (rows || []).map((r) => ({ category: r.type, value: Number(r.num || 0) }));
...
module.exports = { typesStatsWithFilter };
