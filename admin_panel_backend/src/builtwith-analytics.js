...
// DS Q14: SELECT built_with, COUNT(<fk>) AS num FROM <net>_ad_meta_data
//         WHERE built_with_date >= yesterday AND built_with_date < today GROUP BY built_with
//
// LinkedIn (footnote §) reads from linkedin_ad_built_with, not _meta_data.
// FK is bing_text_ad_id / facebook_ad_id / linkedin_ad_id for those three;
// `id` everywhere else.
const DB_DATA = {
...
const builtWithStatsWithFilter = async (req, res) => {
    try {
        ...
        if (built_with) {
            let sql = `SELECT COUNT(${fk}) AS cnt FROM ${tableName} WHERE built_with = ?`;
        ...
        let sql = `SELECT built_with, COUNT(${fk}) AS num FROM ${tableName}`;
        const params = [];
        if (hasRange) {
            sql += ' WHERE built_with_date BETWEEN ? AND ?';
            params.push(fromTs, toTs);
        }
        sql += ' GROUP BY built_with ORDER BY num DESC';

        const rows = await queryDatabase(db_id, index, sql, params);
        const data = (rows || []).map((r) => ({ e_commerce: r.built_with, count: Number(r.num || 0) }));
        ...
        console.error('Error fetching built_with stats:', error);
        return res.status(500).json({ error: 'Internal Server Error' });
    }
};

module.exports = { builtWithStatsWithFilter };
