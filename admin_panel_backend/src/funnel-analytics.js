...
// DS Q15: SELECT built_with_analytics_tracking, COUNT(<fk>) AS num
//         FROM <net>_ad_meta_data
//         WHERE built_with_date >= yesterday AND built_with_date < today
//         GROUP BY built_with_analytics_tracking
//
...
const funnelStatsWithFilter = async (req, res) => {
    try {
        const { funnel, range, network } = req.body;
        ...
        if (funnel) {
            let sql = `SELECT COUNT(${fk}) AS cnt FROM ${tableName} WHERE built_with_analytics_tracking = ?`;
            const params = [funnel];
            if (hasRange) {
        ...
        let sql = `SELECT built_with_analytics_tracking, COUNT(${fk}) AS num FROM ${tableName}`;
        const params = [];
        if (hasRange) {
            sql += ' WHERE built_with_date BETWEEN ? AND ?';
            params.push(fromTs, toTs);
        }
        sql += ' GROUP BY built_with_analytics_tracking ORDER BY num DESC';

        const rows = await queryDatabase(db_id, index, sql, params);
        const data = (rows || []).map((r) => ({
            funnel_key: r.built_with_analytics_tracking,
            count: Number(r.num || 0),
        }));
...
module.exports = { funnelStatsWithFilter };
