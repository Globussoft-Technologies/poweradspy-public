...
// DS Q8: SELECT source, COUNT(id) AS num FROM <net>_ad
//        WHERE first_seen >= yesterday AND first_seen < today GROUP BY source
//
// NOTE: The previous ES implementation grouped by firstSeenOnDesktop/Android/iOS
// field existence on the meta_data document — totally different semantics from
// DS's `source` column. After this change, categories returned will be whatever
// values live in <net>_ad.source.
const DB_DATA = {
...
const adSourceFilter = async (req, res) => {
    try {
        const { source, range, network } = req.body;
        ...
        if (source) {
            let sql = `SELECT COUNT(id) AS cnt FROM ${mainTable} WHERE source = ?`;
            const params = [source];
            if (hasRange) {
        ...
        let sql = `SELECT source, COUNT(id) AS num FROM ${mainTable}`;
        const params = [];
        if (hasRange) {
            sql += ' WHERE first_seen BETWEEN ? AND ?';
            params.push(fromTs, toTs);
        }
        sql += ' GROUP BY source ORDER BY num DESC';

        const rows = await queryDatabase(db_id, index, sql, params);
        const data = (rows || []).map((r) => ({ source: r.source, count: Number(r.num || 0) }));
        ...
        console.error('Error fetching ad source counts:', error);
        return res.status(500).json({ error: 'Internal Server Error' });
    }
};

module.exports = { adSourceFilter };
