...
beforeEach(async () => {
  ...
  modelCalls.length = 0;
  vi.resetModules();
  await import("../../models/competitors_request.js");
});

describe("models/competitors_request", () => {
  it("defines schema with brand_url required + advertiser array", () => {
    const def = schemaCalls[0].def;
    expect(def.brand_url.required).toBe(true);
    expect(Array.isArray(def.advertiser)).toBe(true);
  });
