...
beforeEach(async () => {
  ...
  modelCalls.length = 0;
  vi.resetModules();
  await import("../../models/organic_search.js");
});

describe("models/organic_search", () => {
  it("schema requires domain_name + keyword", () => {
    const def = schemaCalls[0].def;
    expect(def.domain_name.required).toBe(true);
    expect(def.keyword.required).toBe(true);
  });
  it("registers organic_search model", () => {
    expect(modelCalls[0].name).toBe("organic_search");
  });
});
