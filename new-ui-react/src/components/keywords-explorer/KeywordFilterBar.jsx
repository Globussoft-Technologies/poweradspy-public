...
// How long to wait after the user stops typing/changing a field before
// actually applying the filter (re-fetching). Long enough that a normal
// typing cadence never fires mid-word, short enough that it still feels
// live. See KeywordFilterBar's own comment for why this can't be 0 (every
// keystroke as its own request) without reintroducing the race this file
// used to have.
 ...
 *
 *  Every input here edits a LOCAL draft, not the live `filters` prop directly —
 *  typing used to call onChange() (and therefore re-fetch the table) on every
 *  single keystroke, so e.g. typing "laptop" fired 6 separate backend requests,
 *  each racing the others with no cancellation: a slow response for "l" could
 *  land AFTER the "laptop" response and silently overwrite it, making the
 *  filter look like it "wasn't working" even though the last-typed value was
 *  correct. Restored to live-as-you-type per 2026-09-23 request, but fixed
 *  properly this time instead of just reverting: the draft's onChange to the
 *  parent is DEBOUNCED (AUTO_APPLY_DEBOUNCE_MS after the last edit), so a
 *  burst of keystrokes collapses into exactly one fetch — same one-request-
 *  per-deliberate-change property the old Apply button gave, just triggered
 *  by "stopped typing" instead of "clicked a button." Enter still applies
 *  immediately (skips the wait) as an explicit "I'm done" escape hatch. */
const KeywordFilterBar = ({ filters, onChange }) => {
  ...
  // Re-scheduled on every draft change — only the LAST value in a typing
  // burst ever actually fires, which is what prevents the per-keystroke
  // request storm described above.
  useEffect(() => {
    clearPendingApply();
    if (JSON.stringify(draft) === JSON.stringify(filters)) return; // nothing pending
    debounceTimer.current = setTimeout(() => onChange(draft), AUTO_APPLY_DEBOUNCE_MS);
    return clearPendingApply;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [draft]);

  const set = (key, value) => setDraft((d) => ({ ...d, [key]: value === "" ? undefined : value }));
  const applyNow = () => { clearPendingApply(); onChange(draft); };
  const clear = () => { clearPendingApply(); setDraft({}); onChange({}); };
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-3.5 gap-y-3 rounded-2xl border border-theme-border bg-theme-card px-4 py-3 shadow-sm">
      ...
      />
      <Divider />
      <TextInput icon={<Tag size={12} className="text-[#6b99ff]" />} placeholder="Category" tip="Filter to keywords in a specific ad category (exact match)." value={draft.category} onChange={(v) => set("category", v)} onApply={applyNow} />
      <TextInput icon={<Plus size={12} className="text-[#6b99ff]" />} placeholder="Include term" tip="Only show keywords whose text contains this term." value={draft.include} onChange={(v) => set("include", v)} onApply={applyNow} />
      <TextInput icon={<Minus size={12} className="text-[#6b99ff]" />} placeholder="Exclude term" tip="Hide keywords whose text contains this term." value={draft.exclude} onChange={(v) => set("exclude", v)} onApply={applyNow} />
