...
const TimeChart = ({ StatusSystemInfo,loadingStatusSystemInfo, dateRange1, onClose, onStageClick }) => {
  ...
  useEffect(() => {
    if (loadingStatusSystemInfo || !StatusSystemInfo || !StatusSystemInfo.timeline?.length) return;
    const root = am5.Root.new(chartRef.current);
    root._logo?.dispose();
    root.setThemes([am5themes_Animated.new(root)]);
     // Add this line to remove the logo
    root._logo?.dispose();
    ...
    const processedData = StatusSystemInfo.timeline.map(item => ({
      ...item,
...
hoverArea.events.on("pointerout", () => {
  ...
  }, [StatusSystemInfo, filteredStatus,loadingStatusSystemInfo]);
  // Rest of your component code remains the same until the return statement...

  return (
    <div className="p-4 w-full h-full">
      <div className="flex justify-between items-center mb-4">
        <h4 className="text-md font-semibold text-gray-800">System Status Timeline</h4>
        <button 
          onClick={onClose}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-800">
                {formatSecondsToTime(StatusSystemInfo?.totalActive)}
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-red-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-800">
                {formatSecondsToTime(StatusSystemInfo?.totalInactive)}
              </p>
            </div>
