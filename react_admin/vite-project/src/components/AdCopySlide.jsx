...
const AdCopySlide = ({ selectedSession }) => {
  const [AdCopyData, setAdCopyDataData] = useState([]);
  ...
  useEffect(() => {
    ...
    selectedSession?.chats?.forEach((chat, chatIndex) => {
      if (chat?.adCopySide?.length > 0) {
        chat?.adCopySide?.forEach((creative) => {
            newAdCopyData?.push({
            ...creative,
    ...
    setAdCopyDataData(newAdCopyData);
    
    // Initialize expanded states
    const initialExpandedStates = {};
    newAdCopyData?.forEach((_, index) => {
    ...
    <div className="creative-side">
      {AdCopyData?.length === 0 ? (
        ...
        <div className="creative-grid">
          {AdCopyData?.map((adCopy, index) => (
            <div key={index} className="creative-card">
              <div className="creative-header">
                <div>
                  <span className="chat-badge">Chat #{adCopy.chatIndex}</span>
                <span className="platform-badge">
                    ...
                    ) : adCopy?.platform === 'google search ads' || adCopy?.platform === 'google' ? (
                      <img src={Google} alt="Google" width="18" height="18" />
                    ):adCopy?.platform === 'google_performance_max_ads' ? (
                      <img src={Google} alt="google_performance_max_ads" width="18" height="18" />
                    ) : adCopy?.platform === 'google_display_ads' ? (
              ...
              {adCopy?.cta && (
                <div className="creative-cta">
                  ...
                  <div className="cta-value">{adCopy?.cta}</div>
                </div>
              )}
              
              {adCopy?.brandDescription && (
                <div className="brand-description">
                  <div className="description-label">Brand Description</div>
                  <p className="description-text">
                    {adCopy?.brandDescription?.length > 300 && !expandedStates[index]
                      ? `${adCopy?.brandDescription?.substring(0, 300)}...` 
                      : adCopy?.brandDescription}
                  </p>
                  {adCopy?.brandDescription?.length > 300 && (
                    <button 
