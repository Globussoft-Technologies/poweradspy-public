...
const AdvertiserSearches = () => {
  ...
  const handleUserDetails = async (rowData) => {
    const data = {type:1, keyword: rowData.search_advertiser};
  ...
  const [pageSize, setPageSize] = useState(10); 
    async function getAdvertisersData(overrideSearchAfter = searchAfter) {
      setLoading(true);
      const body = {
      ...
      const apiUrl = `${import.meta.env.VITE_NODE_USER_ACTIVITY_API}get-advertiser`;
  ...
  const columns = [
    columnHelper.accessor("search_advertiser", {
      id: "search_advertiser",
      header: "Advertiser",
      cell: (info) => {
    ...
    columnHelper.accessor("search_advertiser", {
      ...
      cell: ({ row }) => {
        ...
        for (const key in data) {
          if (key === "search_advertiser") continue;
    ...
    columnHelper.accessor("action", {
     ...
     columnHelper.accessor("link", {
      ...
      header: "URL",
      cell: ({ row }) => {
        const keyword = row.original.search_advertiser || "unknown";
        const network = row.original.network || "default";
        const baseUrl = import.meta.env.VITE_APP_BASE_URL;
    
        const url = `${baseUrl}/${network.toLowerCase()}/landing/advertiser/${safeEncode(keyword)}`;
...
const advertiserSearchTableref = useRef(null);
    useEffect(() => {
      if(searchdataFilterTable === 0 || searchdataFilterTable === 2 ) {
         const el = advertiserSearchTableref?.current;
    if (!el) return;
  ...
  return (
    <div
      className={`bg-white rounded-[10px] w-full py-[18px] ${searchdataFilterTable === 3 || searchdataFilterTable === 1 ? 'opacity-[100%]' : 'opacity-50' } `}
    ...
    >
      ...
      <div className={`overflow-auto w-full pl-[30px] ${AdvertisersData.length > 0 ? 'h-[365px]' : 'min-h-[200px]'}`} ref={advertiserSearchTableref}>
                  ...
                  className={`h-12 border-b border-border_primary border-[#dddddd] hover:bg-table-row-hover-primary  
                                   ${adsCount === 0 ? 'bg-red-500 text-white' : 
                                     index % 2 === 0
                                       ...
                                       : "bg-white"
                                   } ${searchdataFilterTable === 1 || searchdataFilterTable ===3 ? '' : 'pointer-events-none opacity-50'}`}
                >
...
export default AdvertiserSearches;
