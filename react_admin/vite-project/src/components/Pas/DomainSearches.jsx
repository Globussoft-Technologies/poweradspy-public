...
const DomainSearches = () => {
  ...
  const handleUserDetails =async(rowData) => {
    const data = { type:2, keyword: rowData.search_domain};
  ...
  async function getDomainData(overrideSearchAfter = searchAfter) {
    setLoading(true);
    const body = {
    ...
    const apiUrl = `${import.meta.env.VITE_NODE_USER_ACTIVITY_API}get-domain`;
  ...
  const columns = [
    columnHelper.accessor("search_domain", {
      id: "search_domain",
      header: "Domains",
      cell: (info) => {
        ...
        return (
          <div className="flex items-center space-x-2 break-words w-[94%]">
            <span className="text-[#343A40]  capitalize w-full">
              {user || "N/A"}
            </span>
          </div>
        );
    ...
    columnHelper.accessor("search_keyword", {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-[#343A40]  text-sm h-full overflow-y-auto overflow-x-hidden break-words whitespace-normal">
...
const domainSearchTableref = useRef(null);
        useEffect(() => {
            if(searchdataFilterTable === 0  || searchdataFilterTable === 1) {
           
               const el = domainSearchTableref.current;
          if (!el) return;
  ...
  return (
    <div className={`bg-white rounded-[10px] w-full py-[18px] ${searchdataFilterTable === 3 || searchdataFilterTable === 2 ? 'opacity-[100%]' : 'opacity-50' } `}>
      <div className="pl-[30px] pr-[24px] flex justify-between items-center mb-4">
        <p className="text-[#1f296a] font-[600] text-[24px] ">Domain Searches</p>
      ...
      <div className={`overflow-auto w-full pl-[30px] ${DomainData.length > 0 ? 'h-[365px]' : 'min-h-[200px]'}`} ref={domainSearchTableref}>
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header,id) => (
                  <th
                    key={header.id}
                    className={`table_header_class !px-4 py-3 h-[53px]  text-left text-13 font-medium text-[#343A40]  hover:text-gray-500  transition-all duration-100 ease-in cursor-pointer ${id === 0 ? "md:w-[50%] sm:w-[60%] w-[44%]" : ""}`}
                  >
          ...
          <tbody>
            ...
            table.getRowModel().rows.length > 0 ? (
              table.getRowModel().rows.map((row, index) => {
                ...
                <tr
                  key={row.id}
                  className={`h-12 border-b w-full border-border_primary border-[#dddddd] hover:bg-table-row-hover-primary  
                                   ${adsCount === 0 ? 'bg-red-500 text-white' : 
                                     index % 2 === 0
                                       ? "bg-[#fff] text-gray-700 "
                                       : "bg-white"
                                   } ${searchdataFilterTable === 2 || searchdataFilterTable ===3 ? '' : 'pointer-events-none opacity-50'}`}
                >
                  {row.getVisibleCells().map((cell,index) => (
                    <td
                      key={cell.id}
                      className={`!px-5 !py-3 text-13 text-left font-normal text-[#343A40] h-[42px] ${index === 0 ? "!w-[44%] !sm:w-[54%] !md:w-[42%]" : "max-w-[140px]"}`}                   
                    >
