...
import CreativeSlide from "../../src/components/CreativeSlide.jsx";

const makeSession = (chats) => ({ chats });

describe("CreativeSlide", () => {
  it("returns null when no selectedSession", () => {
    const { container } = render(<CreativeSlide />);
    expect(container.innerHTML).toBe("");
  });
  it("shows empty-state when session has no adCreativeSide", () => {
    const { getByText } = render(
      <CreativeSlide selectedSession={makeSession([{ adCreativeSide: [] }])} />,
    );
    expect(getByText("No creative side data recorded")).toBeInTheDocument();
  });
  it("renders each creative item with Chat # badge", () => {
    const { getByText } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [{ brandName: "Nike", platform: "facebook", timestamp: 1700000000000 }] },
        { adCreativeSide: [{ brandName: "Adidas", platform: "google", timestamp: 1700000000000 }] },
      ])} />,
    );
  ...
  it.each([
    ...
    const { container } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [{ brandName: "X", platform, timestamp: 0 }] },
      ])} />,
    );
  ...
  it("renders meta + twitter + google_video_ads icons (jsdelivr)", () => {
    const { container } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [
  ...
  it("unknown platform → no platform icon rendered", () => {
    const { container, getByText } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [{ brandName: "X", platform: "unknown_x", timestamp: 0 }] },
      ])} />,
    );
  ...
  it("CTA section rendered when cta present", () => {
    const { getByText } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [{ brandName: "X", cta: "Click here", timestamp: 0 }] },
      ])} />,
    );
  ...
  it("CTA section hidden when cta missing", () => {
    const { queryByText } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [{ brandName: "X", timestamp: 0 }] },
      ])} />,
    );
  ...
  it("brandDescription <= 300 chars renders fully, no read-more", () => {
    const { getByText, queryByText } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [{ brandName: "X", brandDescription: "short desc", timestamp: 0 }] },
      ])} />,
    );
    expect(getByText("short desc")).toBeInTheDocument();
    expect(queryByText("Read more")).toBeNull();
  });
  it("brandDescription > 300 chars shows Read more, toggles to Read less", () => {
    const longText = "a".repeat(400);
    const { getByText } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [{ brandName: "X", brandDescription: longText, timestamp: 0 }] },
      ])} />,
    );
  ...
  it("chats without adCreativeSide are skipped", () => {
    const { getByText } = render(
      <CreativeSlide selectedSession={makeSession([
        { adCreativeSide: [] },
        { adCreativeSide: [{ brandName: "Visible", timestamp: 0 }] },
      ])} />,
    );
    expect(getByText("Chat #2")).toBeInTheDocument();
  });
